...
const { createCalendarEvent, updateCalendarEvent, deleteCalendarEvent } = require("./calendarSync");
const { sendPushToOrg, sendPushToUser, createInAppNotification } = require("./sendNotification");
 ...
 */
async function getVehicleCalendarId(vehicleId) {
    if (!vehicleId) return null;
    const snap = await db.collection("vehicles").doc(vehicleId).get();
    if (!snap.exists) return null;
    const calendarId = snap.data().googleCalendarId || null;
 ...
 * 예약 생성 시 -> 캘린더 이벤트 생성
 */
const onReservationCreated = onDocumentCreated("reservations/{reservationId}", async (event) => {
    ...
    try {
        ...
        const eventId = await createCalendarEvent(calendarId, reservation);
        ...
        console.error("Reservation " + reservationId + ": calendar event creation failed", err.message);
    }
    ...
    try {
        ...
        console.error("Reservation " + reservationId + ": push notification failed", err.message);
    }
});
 ...
 */
const onReservationUpdated = onDocumentUpdated("reservations/{reservationId}", async (event) => {
    ...
    try {
        ...
        if (changed) {
            if (calendarId && eventId) {
                await updateCalendarEvent(calendarId, eventId, after);
            ...
            if (after.userId) {
                ...
                var changeMsg = changeParts.length > 0
        ...
        console.error("Reservation " + reservationId + ": calendar event update failed", err.message);
    }
});
 ...
 * 예약 삭제 시 -> 캘린더 이벤트 삭제
 */
const onReservationDeleted = onDocumentDeleted("reservations/{reservationId}", async (event) => {
    const reservation = event.data.data();
    const reservationId = event.params.reservationId;
    ...
    try {
        ...
        console.error("Reservation " + reservationId + ": calendar event delete failed", err.message);
    }
});

module.exports = {
    getVehicleCalendarId,
