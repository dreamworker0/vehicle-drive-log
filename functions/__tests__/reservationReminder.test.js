...
describe('checkReservationReminders', () => {
    ...
    it('예약이 없으면 알림을 보내지 않는다', async () => {
        ...
        mockGet
            .mockResolvedValueOnce(emptySnap) // 1. 임박 예약
            .mockResolvedValueOnce(emptySnap) // 2. 완료 예약
            .mockResolvedValueOnce(emptySnap); // 3. No-show
    ...
    it('10분 이내 예약에 임박 알림을 보낸다', async () => {
        ...
        mockGet
            .mockResolvedValueOnce({ docs: [upcomingDoc] }) // 임박
            .mockResolvedValueOnce({ docs: [] })            // 완료
            .mockResolvedValueOnce({ docs: [] });           // No-show
    ...
    it('종료 후 운행일지가 없으면 미작성 알림을 보낸다', async () => {
        ...
        mockGet
            .mockResolvedValueOnce({ docs: [] })                                  // 임박
            .mockResolvedValueOnce({ docs: [completedDoc] })                      // 완료
            .mockResolvedValueOnce({ docs: [], empty: true })                     // driveLogs 검색 (없음)
            .mockResolvedValueOnce({ docs: [] });                                 // No-show
    ...
    it('미출발(No-show) 예약에 알림을 보낸다', async () => {
        const noShowDoc = {
            ...
            data: () => ({
                ...
                startTime: '09:40', // 20분 전 (>15분 경과, 현재 10:00)
        ...
        mockGet
            .mockResolvedValueOnce({ docs: [] })               // 임박
            .mockResolvedValueOnce({ docs: [] })               // 완료
            .mockResolvedValueOnce({ docs: [noShowDoc] });     // No-show
