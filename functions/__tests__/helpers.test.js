const { log, wrapHttps, wrapHandler } = require('../helpers');
...
describe('helpers — 공통 유틸리티', () => {
    ...
    describe('wrapHttps()', () => {
        it('핸들러를 정상 실행한다', async () => {
            ...
            const res = { json: jest.fn(), status: jest.fn().mockReturnThis(), headersSent: false };
        ...
        it('이미 헤더가 전송된 경우 추가 응답을 보내지 않는다', async () => {
            ...
            const wrapped = wrapHttps('testFn', handler);
            const req = { method: 'GET', path: '/' };
            const res = { json: jest.fn(), status: jest.fn().mockReturnThis(), headersSent: true };
