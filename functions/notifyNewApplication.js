/**
 * 기관 신청 이메일 알림 — Firestore 트리거
 * organizations 컬렉션에 새 문서가 생성되면 슈퍼관리자에게 이메일 전송
 */
...
exports.notifyNewApplication = onDocumentCreated(
    ...
    async (event) => {
        ...
        const orgName = data.name || "이름 없음";
        const applicantName = data.applicantName || "이름 없음";
        const orgEmail = data.applicantEmail || "이메일 없음";
        const orgPhone = data.applicantPhone || "전화번호 없음";
        const appliedAt = data.createdAt
            ? new Date(data.createdAt.seconds * 1000).toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })
        ...
        try {
            ...
            console.error("신청 알림 이메일 전송 실패:", err.message);
        }
    }
);
