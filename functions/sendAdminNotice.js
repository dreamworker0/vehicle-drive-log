/**
 * 공지 알림 — onCall Cloud Function
 * 기관 소속 사용자가 전체에게 공지사항을 전송
 */
const { onCall, HttpsError } = require("firebase-functions/v2/https");
const { getFirestore } = require("firebase-admin/firestore");
const { sendPushToOrg, createInAppNotificationForOrg } = require("./sendNotification");

const db = getFirestore();

exports.sendAdminNotice = onCall(
    { region: "asia-northeast3" },
    async (request) => {
        // 인증 확인
        if (!request.auth) {
            throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
        }

        const uid = request.auth.uid;
        const { orgId, title, message } = request.data;

        if (!orgId || !title || !message) {
            throw new HttpsError("invalid-argument", "orgId, title, message는 필수입니다.");
        }

        // 권한 확인: admin 또는 superAdmin만 허용
        const userDoc = await db.collection("users").doc(uid).get();
        if (!userDoc.exists) {
            throw new HttpsError("permission-denied", "사용자를 찾을 수 없습니다.");
        }

        const userData = userDoc.data();
        if (!["superAdmin", "admin", "employee"].includes(userData.role)) {
            throw new HttpsError("permission-denied", "기관 소속 사용자만 공지를 보낼 수 있습니다.");
        }

        // 자기 기관에만 공지 가능
        if (userData.organizationId !== orgId) {
            throw new HttpsError("permission-denied", "자기 기관에만 공지를 보낼 수 있습니다.");
        }

        try {
            const senderName = userData.name || userData.email || "관리자";

            // 1. Firestore 앱 내 알림 생성 (기관 전체, 본인 포함)
            await createInAppNotificationForOrg(
                orgId,
                "admin_notice",
                title,
                `${senderName}: ${message}`,
                null
            );

            // 2. FCM 푸시 알림 전송 (기관 전체, 본인은 제외)
            await sendPushToOrg(
                orgId,
                {
                    title: `공지: ${title}`,
                    body: message,
                },
                uid
            );

            console.log(`Admin notice sent: org=${orgId}, title="${title}", by=${senderName}`);
            return { success: true };
        } catch (err) {
            console.error("Admin notice failed:", err.message);
            throw new HttpsError("internal", "공지 전송에 실패했습니다.");
        }
    }
);
