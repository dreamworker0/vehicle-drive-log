 ...
 * 화이트리스트인 이유가 셋 있다.
 *
 * 1. `syncNextLogStartKm`의 연쇄 동기화는 한 번의 정정으로 수백~수천 건의 startKm/endKm
 *    쓰기를 만든다(MAX_CHAIN=20은 한 호출만 제한하고, 20번째 문서의 update가 트리거를
 *    재발동해 연쇄가 이어진다). 이를 그대로 기록하면 사람의 편집 몇 건이 시스템 파급 수백
 *    건에 묻혀 제16조 ②의 점검이 실질적으로 불가능해진다. km은 차량 자산 데이터이지
 *    개인정보가 아니므로 화이트리스트에서 자연히 빠진다.
