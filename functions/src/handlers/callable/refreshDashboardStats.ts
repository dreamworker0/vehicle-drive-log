 ...
 * 재집계는 전역 풀스캔(수천~1만+ read)이므로, 연타·다중 superAdmin 동시 클릭이 곧바로 비용으로
 * 이어진다. 야간 배치(02:00)가 매일 자동 갱신하므로 수동 재집계는 배포 직후·대량 작업 직후
 * 예외 상황용이다 — 낮 시간 수동 재집계를 쿨다운 창당 1회로 묶는 방어선 (5분 → 30분 연장,
 * 2026-08: 클릭 1회가 일일 무료 read 할당량의 절반을 소모하는 문제 완화).
 */
const REFRESH_COOLDOWN_SEC = 30 * 60; // 30분
