 ...
 *
 * 이 테스트를 먼저 붙이는 이유는 **런타임 메이저 이관의 전제**이기 때문이다.
 * firebase-functions 7이 번들하는 @types/express가 5 계열로 올라가면 이 파일의
 * `Request`/`Response` named import가 깨진다(백로그 "별도 트랙" 2번). 타입만 바꾸는
 * 작업이지만 관문 로직이 무검증이면 무검증 리팩토링이 된다 —
 * 백로그가 지정한 "기존 단위 테스트로 동작을 고정한 뒤 이관한다"를 충족시킨다.
 *
 * 고정하는 계약:
