...
export async function computeAllDashboardStats(): Promise<void> {
    ...
    // ── 10. Firestore에 3개 문서 batch write ──

    const batch = db.batch();

    batch.set(db.doc("system/dashboardStats"), {
        approvedOrgs,
        totalUsers,
        adminCount,
        employeeCount,
        ...
        favoriteUserRatio: {
            ...
            rate: totalUsers > 0 ? Math.round((withFavCount / totalUsers) * 100) : 0,
        },
        ...
        monthlyStats: {
            ...
            prevActiveUsers: prevMonthActiveUsers.size,
        },
        firstEmployeeStats,
        firstEmployeeDist,
        firstEmployeeTrend,
        onboardingStats: {
            ...
            rate: totalOrgsCount > 0 ? Math.round((onboardingCompleted / totalOrgsCount) * 100) : 0,
        },
        orgSizeDistribution,
    ...
    batch.set(db.doc("system/dashboardTimeSeries"), {
        dailyDriveStats,
        dailyActiveUserStats,
        dailyActiveOrgStats: dailyOrgData,
        inputMethodStats,
        favoriteStats: favoriteStatsArr,
        dailyAvgDuration,
        hourlyStats,
        hourlyAvgDuration,
        heatmapData: {
    ...
    batch.set(db.doc("system/dashboardOrgRankings"), {
        topOrgs,
        orgAvgDuration,
        funnelData,
        lastUpdatedAt: new Date().toISOString(),
    ...
    console.log(`[computeDashboardStats] 완료: ${elapsed}ms, orgs=${approvedOrgs}, logs=${totalLogs}, users=${totalUsers}`);
}
