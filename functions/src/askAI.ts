...
export const askAI = onCall(
    ...
    async (request) => {
        ...
        // Rate limit: 1분에 5회
        await checkRateLimitByUid("askAI", uid, 5, 60);
...
답변:`;
            ...
            const answer = response.text?.trim() || "죄송합니다, 답변을 생성하지 못했습니다. '건의하기'를 통해 개발자에게 직접 문의해주세요.";
            ...
            console.error("[askAI] 오류:", error.message);
            throw new HttpsError("internal", "답변 생성 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
        }
    }
);
