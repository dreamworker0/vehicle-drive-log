...
// Gemini API 키를 Firebase 환경변수에서 가져오기
const geminiApiKey = defineString("GEMINI_API_KEY");

/**
 * 계기판 OCR — 카메라로 촬영한 계기판에서 누적Km(+배터리%) 추출
 * 
 * @param {object} data - { imageBase64: string, mimeType: string, isElectric: boolean }
 * @returns {{ km: number|null, battery: number|null, raw: string }}
 */
exports.ocrDashboard = onCall(
    {
...
값을 확인할 수 없는 경우 null로 표시해주세요.`;
            ...
            // JSON 파싱 시도
            let result = { km: null, battery: null };
            try {
                // 응답에서 JSON 부분만 추출 (```json ... ``` 형태 대응)
