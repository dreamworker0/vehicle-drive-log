---
name: data-export-pattern
description: PDF(인쇄 레이아웃) 및 Excel(xlsx 동적 로딩) 파일 내보내기 구현 및 갱신 패턴 가이드
---

<!-- AUTO-GENERATED — scripts/sync-claude-agents.ts가 .agent/skills/data-export-pattern/SKILL.md에서 생성. 직접 수정 금지. -->

# data-export-pattern

> 📄 **단일 원본**: [.agent/skills/data-export-pattern/SKILL.md](../../../.agent/skills/data-export-pattern/SKILL.md)
> 이 파일은 Claude Code 자동 발동을 위한 포인터다. 내용 수정은 원본에서 하고 `npm run sync:agents`로 재생성한다.

PDF(인쇄 레이아웃) 및 Excel(xlsx 동적 로딩) 파일 내보내기 구현 및 갱신 패턴 가이드

전체 패턴·예시·체크리스트는 위 원본 파일을 읽고 따른다.
