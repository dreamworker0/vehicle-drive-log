---
name: add-component
description: 프로젝트 구조와 네이밍 컨벤션에 맞게 새 React 컴포넌트를 추가하는 가이드
---

<!-- AUTO-GENERATED — scripts/sync-claude-agents.ts가 .agent/skills/add-component/SKILL.md에서 생성. 직접 수정 금지. -->

# add-component

> 📄 **단일 원본**: [.agent/skills/add-component/SKILL.md](../../../.agent/skills/add-component/SKILL.md)
> 이 파일은 Claude Code 자동 발동을 위한 포인터다. 내용 수정은 원본에서 하고 `npm run sync:agents`로 재생성한다.

프로젝트 구조와 네이밍 컨벤션에 맞게 새 React 컴포넌트를 추가하는 가이드

전체 패턴·예시·체크리스트는 위 원본 파일을 읽고 따른다.
