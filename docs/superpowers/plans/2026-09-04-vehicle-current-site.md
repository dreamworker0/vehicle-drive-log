...
- 분관을 등록하지 않은 기관의 화면·내보내기 파일은 **한 픽셀도 바뀌지 않아야 한다.**
- 분관은 등록했지만 전 차량이 고정인 기관에서 달라지는 것은 **오늘의 예약 카드에 붙는 위치 이름 한 줄뿐**이다(`🚩 제2분관`, 확인 시각 없음). 그 차를 처음 쓰는 직원에게 필요한 정보이고 `resolveStartLocationLabel`이 이미 같은 이유로 라벨을 기록한다. 입력 항목·저장 값·내보내기·경로 계산은 전부 그대로여야 한다.
