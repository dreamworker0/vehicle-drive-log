...
> 2026-08-09 기준 `firestore.indexes.json`의 복합 인덱스는 **총 37개**다. 아래 표는 2026-03 스냅샷(22개)이므로
> 나머지 15개를 포함한 정확한 매핑·미사용 판정은 GCP Console → Firestore → 인덱스 탭의 실제 히트 수로 재검증이 필요하다.
>
...
### 검토 대기
3. **인덱스 37개 전수 재검증** — GCP Console 실제 히트 수로 미사용 인덱스 판정 후 제거 (§1 참조)
