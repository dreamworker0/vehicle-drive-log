...
|---|---|---|---|
| `nightlyStatsBatch` | 매일 02:00 KST | 기관 월간 집계 + 대시보드 통계 캐시 — 기관·사용자·차량·운행일지를 한 번만 읽어 두 단계가 공유 | **큼** — 2026-09-25 실측 하룻밤 약 2.4만 read(하루 읽기의 절반). 공유 전에는 두 단계가 같은 원본을 각자 읽고 월간 집계가 기관마다 쿼리 6종을 던졌다 |
