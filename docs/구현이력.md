...
| **판정 단일화** | `vehicleUtils.isVehicleRestrictedForUser(vehicle, uid, role)` 공용 헬퍼 — 목록 없음/빈 배열·admin/superAdmin·목록 포함이면 false. UI 4곳·제출 검증이 모두 이 헬퍼 하나로 판정 |
...
| **직원 UI** | 두 VehicleSelector(공용 예약·employee 운행일지/빠른운행)에서 제한 차량을 숨기지 않고 🔒 "지정 차량" 배지 + disabled(정비 중 패턴 확장). 컴포넌트 내부 `useAuth()`로 uid/role 취득해 prop drilling 회피 |
| **서버 강제** | `createReservationSafe` 트랜잭션 내 기존 차량 org 검증(N3) 직후 허용 목록 검사 — 이미 읽는 vehicleSnap 재사용이라 추가 read 0. caller uid 기준(서버가 `reservedByUid=auth.uid` 강제 저장이라 검사·저장 일치), admin/superAdmin role 클레임은 bypass. 위반 시 `permission-denied` |
...
| **검증** | 프론트 lint·tsc·build(번들 예산 이내)·Vitest 469/469, functions tsc·Jest 232/232(신규 allowedUserIds 4케이스: 미포함 거부·포함 허용·admin bypass·빈 배열 전체 허용). vehicleUtils 헬퍼 테스트 5케이스 추가 |
