...
| **배포 후 유의** | orgStats/monthly는 야간 배치(`runDailyAggregation`)만 기록하며 배포 직후엔 당월+전월만 갱신된다. 분석 대시보드 기본 6개월 창의 과거 월을 즉시 교정하려면 `runDailyAggregation(6)` 형태의 **1회 백필**을 의도적으로 트리거해야 함(별도 후속). 2개월 재집계로 기관당 야간 읽기 쿼리 약 6→10개로 증가(1일 1회, 허용 범위) |
...
| **트리거 방식** | 영구 UI 없이 superAdmin이 로그인된 상태에서 1회 호출(브라우저 콘솔의 `httpsCallable`). 일회성 데이터 교정이므로 전용 화면은 과설계로 판단 |
...
| **후속(운영)** | 배포 후 superAdmin이 `backfillMonthlyStats({months:6})` 1회 실행 → 과거 6개월 orgStats/monthly 재집계. 이후 야간 배치가 당월+전월을 계속 유지 |
...
| **검증** | functions tsc·lint·테스트(프로듀서 6 + 백필 5) 통과 후 배포, 백필 재실행 → 함수 로그로 실제 기관별 집계 성공 확인 |
