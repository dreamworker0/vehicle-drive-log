...
| 항목 | 내용 |
|------|------|
| **신규 테스트** | `src/__tests__/lib/firestore/vehicles.test.ts`(10건)·`users.test.ts`(18건). 조회는 **`where('organizationId','==',orgId)` org 격리 단언 필수**(절대 규칙 1), 변경 함수는 캐시 무효화(`invalidateCache('vehicles')`)·`captureError` 보고+재던짐 고정 |
...
| **검증** | Node 22로 tsc·ESLint 0 오류·스코프 테스트 32/32(기존 `firestore.test.ts` 포함, 중복 없음 확인 — 기존 파일은 트랜잭션 롤백·getDriveLogs 에러 핸들링 담당). 커밋 `a5b00c5`(테스트만, 소스 무변경) |
