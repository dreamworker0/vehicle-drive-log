...
> 2026-07-11, Claude 재평가(84.2점)에서 코덱스 개선계획 작업 2·3 + 신규 지적 2건(PII 평문 로그, defineSecret 미사용)을 조치. App Check 2·3배치는 Authentication 미검증 30%(인앱 브라우저 App Check 미초기화 경로 확인)로 보류 — 함수별 메트릭 확인 후 진행.
...
| **defineSecret 이관** | GMAIL_APP_PASSWORD·EMAILJS_PRIVATE_KEY·ALIMTALK_PROXY_TOKEN·GOOGLE_OAUTH_CLIENT_SECRET을 `core/params.ts` 중앙 defineSecret로 이관, 소비 함수 12곳 `secrets` 바인딩(이메일 4·알림톡 5·autoVerifyDocument·예약 트리거 3종). 서비스 코드는 process.env 읽기 유지(바인딩 시 주입) |
| **커밋 분리** | `27e2b21`(마스킹·MIME·fail-closed — 즉시 배포 가능) / `7c15fb6`(defineSecret — **시크릿 4종 사전 등록 + functions/.env 동일 키 제거 전 푸시 금지**, 미이행 시 CI Deploy 실패) |
| **검증** | Functions 31 suites/284 테스트(신규 3파일 포함)·tsc·빌드·ESLint 0오류, Node 22 |
| **배포 전 필수** | `firebase functions:secrets:set` 4종(GMAIL_APP_PASSWORD·EMAILJS_PRIVATE_KEY·ALIMTALK_PROXY_TOKEN·GOOGLE_OAUTH_CLIENT_SECRET) → functions/.env에서 동일 키 제거 → 푸시 |
