...
| **검증** | 로컬에서 CI가 실행할 명령 그대로 `node --experimental-strip-types scripts/security-audit.ts` exit 0 · YAML 파싱으로 잡·의존 관계 확인 · 하네스 Doctor 0건. PR #115에서 **`security-audit`이 `ci`와 나란히 독립 체크로 등장**하고 8초에 통과함을 실측 |
| **남은 것** | ① vite 8 마이그레이션(청크 전략 재작성) — 별건 ② `deploy.yml`의 감사 스텝은 긴급 우회 경로 전용이라 그대로 뒀다 ③ 이 회차는 CI 구조만 봤다. 같은 "조용한 미실행"이 스케줄 워크플로(`functions-coverage.yml`)나 pre-push 훅에도 있는지는 확인하지 않았다 |
