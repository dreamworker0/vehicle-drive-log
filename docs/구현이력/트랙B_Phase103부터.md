...
### Phase 136: 게이트가 조용히 건너뛰어지던 구조 해소 — 감사 분리 · 마스킹 방지 🛡️ (커밋·PR)
...
| **④-1 왜 그냥 `edited`를 빼지 않았나** | 깔끔한 해법은 `edited` 트리거를 ci.yml에서 빼고 base 변경만 따로 재발화시키는 것인데, **GITHUB_TOKEN으로 한 close/reopen은 워크플로를 재발화시키지 않는다**(PAT 필요). Phase 130이 `edited`를 넣은 이유(스택 PR의 base 재지정 시 CI 미실행, #92·#93)는 그대로 유효하므로 트리거는 유지하고 부작용만 격리했다 |
| **검증** | 로컬에서 CI가 실행할 명령 그대로 `node --experimental-strip-types scripts/security-audit.ts` exit 0 · YAML 파싱으로 잡·의존 관계 확인 · 하네스 Doctor 0건. PR #115에서 **`security-audit`이 `ci`와 나란히 독립 체크로 등장**하고 8초에 통과함을 실측. ④는 GitHub 표현식의 느슨한 비교(`null == ''`)를 흉내낸 스크립트로 이벤트 6종에 대해 **"그룹이 `meta`인 경우 = 잡이 전부 건너뛰어지는 경우"가 정확히 일치**함을 확인 — 진짜 실행이 격리되는 일도, 건너뛰는 실행이 진짜 실행을 죽이는 일도 없다 |
| **남은 것** | ① vite 8 마이그레이션(청크 전략 재작성) — 별건 ② `deploy.yml`의 감사 스텝은 긴급 우회 경로 전용이라 그대로 뒀다 ③ 이 회차는 CI 구조만 봤다. 같은 "조용한 미실행"이 스케줄 워크플로(`functions-coverage.yml`)나 pre-push 훅에도 있는지는 확인하지 않았다 ④ **잡을 `if`로 건너뛰어도 그 이름의 체크 런은 `skipped`로 생성되고, 필수 체크는 skipped를 성공으로 취급한다.** 같은 커밋에서 앞선 실행이 실패해 있었다면 본문 수정만으로 빨간 PR이 초록으로 보일 수 있다. 격리는 "진짜 실행이 죽는 것"만 막았지 이 덮어쓰기는 막지 못한다 — PAT 없이는 해결이 막혀 있어 별건으로 남긴다 |
