...
| **조치 ② 테스트가 잡아낸 두 번째 건** | 회귀 가드를 붙이자마자 실패했다 — 추천 예약 **접힘 상태의 플로팅 버튼이 `z-[90]`**이라 헤더는 물론 **모달(50)까지 덮고 있었다.** 펼침 배너와 같은 층인 `z-40`으로 내렸다. 하단 내비(30) 위, 헤더(45)·모달(50) 아래다 |
...
| **검증** | 실제 Chromium으로 `sticky`+`backdrop-filter` 헤더 안의 z-50 패널과 바깥 z-40 배너를 재현해 **겹치는 좌표에서 `elementFromPoint`가 무엇을 반환하는지** 확인 — z-30이면 배너, z-45면 패널. 값 순서 가드는 **변형 4종(헤더 z-30 회귀 · 헤더 z-50 과상향 · 접힘 버튼 z-90 회귀 · 재동의 배너 z-46 상향)을 모두 실패로 잡음**을 확인했다. 전체 단위 테스트 1,044건 통과 |
