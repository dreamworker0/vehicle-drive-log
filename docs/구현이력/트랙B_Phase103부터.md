...
### Phase 138: E2E 실패 증거가 스스로를 지우던 문제 — 그리고 부팅 청크 실패의 무한 로딩 🔍 (커밋·PR)
...
| **곁가지 — 부팅 청크 실패의 무한 로딩** | 플레이크 기전을 실험하다 별개의 실제 결함을 찾았다. 랜딩 청크(`LandingPage-*.js`)를 **두 번 연속 실패시키면 화면이 "로딩 중..."에서 10초를 넘겨 멈추고**, `Failed to fetch dynamically imported module`이 uncaught로 남는다. 1회 실패는 1.1초 더 걸릴 뿐 정상 복구된다 |
...
| **남은 것** | ① 위 부팅 결함 수정 — `main.tsx`의 엔트리 import에 `catch`를 달아 재시도 UI를 띄워야 한다. 첫 화면이 영원히 "로딩 중..."인 것은 모바일 회선이 불안정한 이용 환경에서 실사용자에게도 일어난다. **별건으로 분리** ② 플레이크 자체의 확정 원인은 여전히 미상이다. 이제 증거가 남으므로 다음 발생 때 trace로 판정한다 |
