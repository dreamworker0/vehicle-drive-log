...
| **PRIVACY_VERSION 판단 → Phase 133에서 올렸다** | 이 회차에서는 시행일(2026-08-05)이 아직 오지 않아 **시행 전 보완**으로 보고 버전을 유지했다. 다만 8/1~8/4에 동의한 기관은 접속기록 항목이 추가되기 **전** 문안에 동의한 것이 되므로 운영자 판단을 요청했고, **재동의를 받는 쪽으로 결정**되어 Phase 133에서 시행일을 옮겼다 |
...
### Phase 133: 약관·처리방침 시행일 재지정 (2026-08-05 → 08-10) — 접속기록 항목 재동의 📜 (커밋·PR)

> 2026-08-01, Phase 132가 운영자 판단으로 남긴 건이 "재동의를 받자"로 결정됐다. 이 회차의 실질은 상수 두 줄이지만, **어느 값을 함께 옮겨야 하는가**가 판단의 전부였다.

| 항목 | 내용 |
|------|------|
| **왜 올리나** | Phase 132가 처리방침 제1조에 접속기록(계정·일시·접속지 IP·접속 환경) 항목을 신설했다. 8/1~8/4에 동의한 기관은 **이 항목이 없는 문안**에 동의한 것이 되므로, 시행일을 옮겨 바뀐 내용으로 다시 확인받는다. `ConsentGate`가 저장된 버전과 현행 상수를 비교하므로 코드 변경 없이 관리자에게 동의 창이 다시 뜬다 |
| **왜 둘 다 옮기나** | `PRIVACY_VERSION`만 올리면 **약관 제9조(위탁)는 8/5 시행, 처리방침 제7·8조(위탁·국외 이전)는 8/10 시행**이 되어, 그 5일간 "약관은 위탁을 규정하는데 처리방침 구버전에는 수탁자 고지가 없는" 구간이 생긴다. `PrivacyPage.tsx`의 기존 주석이 이미 "본문은 이용약관 제9조와 짝을 이루므로 함께 개정해야 한다"고 적고 있었다 — 코드가 스스로 답을 갖고 있던 경우다 |
| **날짜 선택** | 2026-08-10(월). 오늘(8/1, 토) 기준 **9일 유예**로 처리방침 변경 최소 7일 전 공지 관행을 넘고, 영업일이라 문의 대응이 가능하다 |
| **불변식을 테스트로 고정** | "두 시행일은 같다"를 `legalVersions.test.ts`에 단정으로 넣었다. 주석만으로는 다음 개정 때 한쪽만 올려도 아무것도 실패하지 않는다 |
| **직원 배너도 다시 뜬다** | `TERMS_VERSION`이 함께 바뀌므로 직원 비차단 배너가 재노출된다. 약관 **본문**은 안 바뀌었으나 시행일이 바뀐 것은 직원도 확인할 사항이고, 차단이 아니라 배너라 업무를 막지 않는다 |
| **파생 표기 자동 갱신** | 약관·처리방침 페이지의 시행일·개정 이력, 인증 E2E 시드(`e2e/emulator/seed.ts`)가 전부 상수에서 파생되므로 하드코딩 수정이 없다. Phase 130이 만든 이 구조 덕분에 상수 두 줄로 끝났다. 타입 주석의 `2026-08-05` 하드코딩만 `TERMS_VERSION` 참조로 바꿨다 |
| **업데이트 소식** | 기존 8/5 표기를 8/10으로 고치고, **이미 동의한 관리자에게 동의 창이 한 번 더 뜬다**는 사유를 항목으로 추가했다. 설명 없이 두 번째 모달이 뜨면 오작동으로 보인다 |
| **검증** | `verify:fast` · 프론트 **1022건**(시행일 일치 불변식 1건 신설) · 프로덕션 빌드(번들 예산 이내) · 하네스 Doctor 0건. **에뮬레이터 E2E 10/11** — 실패 1건(엑셀 내보내기 파일명)은 컨테이너에 설치된 Chromium(1194)과 프로젝트 Playwright(1.62, 빌드 1234)의 불일치를 심볼릭 링크로 대체한 데서 온 환경 문제다. `recordExport` 계측을 제거하고 재실행해도 **동일하게 실패**해 코드 원인이 아님을 확인했다(`suggestedFilename`이 `download`로 떨어짐). CI는 정규 브라우저를 쓰므로 영향 없다 |
| **남은 것 → Phase 134에서 처리** | 시행일 도래(8/10) 전 사전 공지. 앱 내 공지로 하기로 결정돼 Phase 134에서 발송 경로를 신설했다 |

---

### Phase 134: 전체 기관 일괄 공지 (superAdmin) — 8/10 시행 사전 공지 경로 📢 (커밋·PR)

> 2026-08-01, Phase 133이 남긴 사전 공지를 **앱 내 공지**로 하기로 정하고 나서야 드러난 사실 — 그 경로가 없었다. `sendAdminNotice`는 기관 관리자가 자기 기관에만 보내는 구조(`userData.organizationId !== orgId`이면 거부)이고 superAdmin은 소속 기관이 없어 호출 자체가 막힌다. 이 회차의 핵심은 팬아웃 기능을 만든 것이 아니라, **되돌릴 수 없는 대량 행위를 어떻게 실수하기 어렵게 만드는가**다.

| 항목 | 내용 |
|------|------|
| **문안·채널 검토** | 이메일·알림톡·앱 내 세 안을 검토했다. 알림톡은 승인된 템플릿이 3종(승인·미활성 리마인더·반려)뿐이라 개정 공지용은 **카카오 사전 승인부터** 받아야 해 8/10까지 촉박하고, 일괄 이메일은 발송 함수 자체가 없다. 앱 내 공지로 결정 |
| **2단계 강제** | 발송 버튼을 바로 두지 않는다. `dryRun`이 **대상 수만 반환**하고 화면이 그 인원을 표시한 뒤에만 [발송]이 열린다. 운영자가 "몇 명에게 가는지 모르고" 누르는 상황을 없앤다 |
| **확인한 문안 = 보내는 문안** | 제목·본문을 고치면 확인 결과를 **버린다**. 이 화면에서 가장 위험한 실수는 A를 확인하고 B를 보내는 것이다. 테스트로 고정했다 |
| **중복 발송 차단** | 문서 ID를 `broadcast_{noticeId}_{uid}`로 고정한다. 재클릭·재시도가 같은 사람에게 같은 공지를 두 번 띄우면 오작동으로 읽힌다. 반대로 **의도적 재발송은 막지 않는다** — 화면이 발송마다 새 `noticeId`를 만든다 |
| **기존 헬퍼를 재사용하지 않은 이유 — 읽기 N+1** | `sendPushToOrg`·`createInAppNotificationForOrg`는 기관 단위라 200여 기관을 돌면 ① users 쿼리가 기관 수만큼 반복되고 ② `sendPushToUser`가 uid마다 users 문서를 **다시 읽어** 수천 회의 불필요한 읽기가 생긴다. users를 한 번만 훑고 그때 얻은 `fcmToken`으로 `sendEach`(멀티캐스트)를 쓴다 |
| **순서가 안전장치다** | 앱 내 알림을 **먼저** 커밋하고 그다음 푸시한다. 뒤집으면 푸시를 받고 들어온 사용자의 알림함이 비어 있을 수 있다. 커밋 2회 → 푸시 2회 순서를 테스트로 고정했다 |
| **푸시 실패는 삼킨다** | 청크 실패가 전체를 되돌리면 일부만 받은 상태에서 재발송해야 한다. 앱 내 알림은 이미 남았으므로 성공/실패 수만 세어 돌려주고 ERROR 로그를 남긴다 |
| **500건 상한** | Firestore 배치·FCM `sendEach` 모두 500이 상한이다. 1,201명 → 커밋 500·500·201, 1,100명 → 푸시 500·500·100을 실측으로 고정했다. 상한을 넘기면 조용히 일부만 발송된다 |
| **수신 대상에서 뺀 것** | 비활성(`status: 'disabled'`) 계정 — 로그인할 수 없으므로 알림을 남길 이유가 없다. 기관 미소속(superAdmin·초대 코드 입력 대기) — 기관 공지의 수신자가 아니다. `status` 필드가 없는 구 문서는 활성으로 본다(비활성은 명시적으로만 기록된다) |
| **서버 필터를 안 쓴 이유** | `where('organizationId', '!=', null)`은 단일 필드 색인 정책에 묶이는데, 거의 모든 사용자가 기관에 속해 있어 걸러지는 문서가 적다. 절약되는 읽기가 없어 전량 조회 후 코드에서 거른다 |
| **기본 문안** | 화면 기본값이 8/10 시행 공지다. 시행일은 `TERMS_VERSION`에서 파생시켜 상수와 본문이 어긋날 수 없게 했다(Phase 133이 세운 불변식의 연장) |
| **검증** | Functions **668건**(`sendBroadcastNotice` 21건 신설) · 프론트 **1031건**(화면 9건 신설) · 프로덕션 빌드 · 함수 카탈로그 67개 · 하네스 Doctor 0건 |
| **남은 것** | ① 발송 이력 미기록 → **Phase 135에서 처리** ② 알림톡 템플릿 사전 승인(별건) ③ 일괄 이메일 발송 경로(별건) |

---

### Phase 135: 전체 공지 발송 이력 — 기록 시점이 설계의 전부 📋 (커밋·PR)

> 2026-08-01, Phase 134가 "발송 이력이 Functions 로그에만 있다"로 남긴 건. 기능 자체는 컬렉션 하나와 목록 하나지만, **언제 쓰느냐**가 이 회차의 실질이다. 늦게 쓰면 이력이 없는 발송이 생기고, 그 공백은 곧 중복 발송으로 이어진다.

| 항목 | 내용 |
|------|------|
| **왜 필요한가** | 운영자가 화면에서 "언제 무엇을 몇 명에게 보냈는가"를 볼 수 없으면 재발송 판단을 할 수 없다. Functions 로그는 콘솔에 들어가야 보이고 보존 기간도 짧다 |
| **기록 시점 — 되돌릴 수 없는 지점 직후** | 푸시까지 끝난 뒤 한 번만 쓰면, 그 사이 함수가 죽었을 때 **알림은 나갔는데 이력은 없는** 상태가 된다. 운영자는 보낸 적 없다고 판단해 다시 보내고, 수천 명이 같은 공지를 두 번 받는다. 그래서 앱 내 알림 커밋 **직후** `status: 'sending'`으로 먼저 남기고, 푸시 결과를 `merge`로 덧써 `'sent'`로 마감한다 |
| **중간 상태를 숨기지 않는다** | 푸시 도중 죽으면 `sending`으로 남는다. 이것은 "보냈는지 모른다"가 아니라 **"알림은 나갔고 푸시 결과만 모른다"**를 뜻하며, 화면도 그대로 `푸시 결과 미확인`으로 표시한다. 임의로 `sent`로 덮으면 운영자가 푸시가 나간 줄 알고 후속 판단을 그르친다 |
| **실패 처리를 두 방향으로 갈랐다** | 시작 이력 쓰기 실패는 **그대로 던진다** — 아직 푸시 전이라 피해가 앱 내 알림에 그치고, 이력 없는 발송을 만들지 않는 편이 낫다. 결과 갱신(2회차) 실패는 **삼킨다** — 이미 나간 알림을 되돌릴 수 없는데 호출자에게 실패로 보이면 재발송을 유도한다. 둘 다 테스트로 고정했다 |
| **Rules — 운영자조차 쓸 수 없다** | 읽기는 superAdmin 한정(발송자 uid가 담겨 공개할 이유가 없다), 쓰기는 `if false`. 운영자가 쓸 수 있으면 발송하지 않은 이력을 만들거나 지울 수 있어 기록의 의미가 사라진다. Rules 테스트에서 읽기 3방향·쓰기 3방향(create·update·delete)을 모두 막는 것을 확인했다 |
| **전역 도메인** | `broadcasts`는 기관 경계가 없어 `organizationId` 필터가 없다 — 서비스 운영 기록이지 기관 데이터가 아니다. 커스텀 ESLint 규칙 `local/require-organization-filter`의 적용 파일 목록에 넣지 않은 이유가 이것이다 |
| **배럴 3곳 등록** | 타입(`types/index.ts`)·스키마(`schemas/index.ts`)·데이터 접근(`lib/firestore/index.ts`)에 모두 등록했다. Phase 123에서 `schemas/auditLog.ts`가 스스로 "스키마 누락 결함을 겪었다"고 경고하면서 정작 배럴에 없던 전례가 있다 — `createZodConverter`는 스키마가 모르는 키를 조용히 제거하므로 누락되면 저장은 되고 조회만 안 된다 |
| **목록 조회 상한** | 최근 20건. 상한이 없으면 운영 기간이 길어질수록 화면 진입 비용이 조용히 늘어난다. 이력 조회가 실패해도 발송 화면은 계속 쓸 수 있게 했다(부수 기능이 본 기능을 막지 않는다) |
| **거짓 녹색 가드에 걸렸다** | 이력 로딩이 비동기 `useEffect`라 동기 테스트 3건에서 상태 갱신이 테스트 종료 뒤에 일어나 `act()` 경고가 났고, 이 저장소의 가드가 그것을 실패로 승격시켰다. 렌더 후 초기 로딩 완료를 기다리는 헬퍼(`renderSettled`)로 해소 — 가드가 실제로 작동함을 다시 확인한 셈이다 |
| **검증** | Functions **675건**(이력 7건 추가) · 프론트 **1037건**(화면 6건 추가) · Firestore Rules 20 → **21건** · 프로덕션 빌드 · 카탈로그 재생성 · 하네스 Doctor 0건 |
| **남은 것** | ① 이력 보존 기간 정책 없음 — 저용량이라 당장 문제는 아니나 수년 뒤 목록 조회가 무거워지면 TTL 검토 ② 알림톡 템플릿 사전 승인(별건) ③ 일괄 이메일 발송 경로(별건) |

---

### Phase 136: 게이트가 조용히 건너뛰어지던 구조 해소 — 감사 분리 · 마스킹 방지 · 재검증 분리 🛡️ (커밋·PR)

> 2026-08-02, "감사가 계속 작동을 못한다"는 신고에서 출발했다. 확인해 보니 **감사는 실패하고 있던 게 아니라 도달하지 못하고 `skipped` 되고 있었다.** 이 회차의 요지는 특정 게이트를 고친 것이 아니라, **게이트를 순서에 매달아 두면 앞의 실패가 뒤의 검증을 통째로 삼킨다**는 구조를 걷어낸 것이다.

| 항목 | 내용 |
|------|------|
| **증상** | 열려 있던 Dependabot PR 3건에서 `Security audit`이 전부 `skipped`. 감사는 `ci` 잡의 **24번째(마지막) 스텝**이라 앞 스텝이 하나라도 깨지면 뒤가 전부 건너뛰어진다 |
| **실패 지점은 제각각, 결과는 동일** | #85 eslint 10 → `npm ci` ERESOLVE(`eslint-plugin-jsx-a11y@6.10.2` peer가 eslint 9에서 막힘, 1초) · #86 vite 8 → `Build` 실패(Rolldown 전환으로 `manualChunks is not a function`, 2초) · #88 plugin-react 6 → vite 8 peer 요구(14초). 공통점은 **감사에 도달하지 못했다**는 것뿐 |
| **왜 위험한가** | 체크에는 "설치 실패"로만 뜨고 **감사가 돌지 않았다는 사실은 어디에도 남지 않는다.** 의존성을 올리는 PR이야말로 감사가 가장 필요한 자리인데 정확히 거기서 빠졌다. Phase 114가 감사 스크립트를 fail-closed로 하드닝했지만 **그 하드닝은 스크립트가 실행됐을 때만 작동한다** — 실행 자체가 막히는 경로는 하드닝 바깥에 있었다 |
| **조치 ① 감사를 독립 잡으로** | `needs` 없는 `security-audit` 잡으로 분리. 다른 잡이 어떻게 되든 자기 결론을 낸다. Deploy는 CI 워크플로 **전체 결과**에 종속되므로 감사 실패가 배포를 막는 성질은 유지된다 |
| **①-1 `npm ci`를 두지 않았다** | `npm audit`은 `package-lock.json`만으로 동작하고(node_modules 없는 임시 디렉터리에서 실증) `security-audit.ts`는 Node 내장 모듈만 쓴다. Node 22의 `--experimental-strip-types`로 직접 실행해 **설치 단계 자체를 없앴다** — 설치를 두면 같은 결합이 되살아난다. 실측 8초 |
| **조치 ② 마스킹 방지** | 기본 동작("앞이 실패하면 뒤는 skipped")은 한 번에 하나씩만 드러내, 16분짜리 CI를 실패 개수만큼 반복하게 만든다(Actions 분량이 그만큼 샌다). **실제 선행 산출물이 필요한 스텝만** 그 산출물에 묶고 나머지는 `!cancelled()`로 각자 결론을 내게 했다. 설치 필요 → `steps.install.outcome`, 빌드 산출물 필요 → `steps.build.outcome` |
| **②-1 번들 예산을 빌드 직후로** | `Bundle size check`가 E2E 뒤(끝에서 두 번째)에 있어 **E2E 플레이크 한 번에 예산 초과가 통째로 묻혔다.** 이 저장소는 실제로 번들 예산 초과로 master가 막힌 적이 있고(#112) E2E 플레이크도 반복해 겪었다(Phase 115·122). 둘을 묶어 둘 이유가 없어 빌드 직후로 옮기고 빌드 성공에만 묶었다 |
| **②-2 Rules 테스트 분리** | 멀티테넌트 격리의 **유일한** 자동 검증인데 단위 테스트 실패에 묶여 있었다. 단위 테스트 하나가 깨지면 기관 간 데이터 격리 회귀가 그 실행에서 아예 검사되지 않는다 — 가장 묻히면 안 되는 게이트라 설치 성공에만 묶었다 |
| **진짜 의존은 남겼다** | E2E는 빌드 산출물이, 번들 체크는 `dist/`가 실제로 필요하다. 이런 것까지 떼면 "빌드가 없는데 검사했다"는 무의미한 실패가 난다. 위치 때문에 묶인 것과 실제로 의존하는 것을 구분하는 것이 이 작업의 전부다 |
| **조치 ③ 올릴 수 없는 메이저 보류** | eslint·vite·`@vitejs/plugin-react` 메이저를 Dependabot `ignore`. 열어 두면 매주 같은 PR이 되살아나 CI를 빨간 상태로 점유하고, **실패가 일상이 되면 진짜 실패를 못 알아본다.** 영구 차단이 아니라 **재검토 조건을 주석으로 명시한 보류** — eslint는 `jsx-a11y`가 peer를 넓히면, vite 계열은 청크 전략을 함수형·`advancedChunks`로 재작성하면 해제. PR 3건은 원인·해제 조건을 코멘트로 남기고 닫았다 |
| **조치 ④ 메타 편집 실행의 concurrency 격리** | 고치는 도중 같은 병이 한 번 더 나왔다. **PR 본문을 고쳤더니 진행 중이던 진짜 CI(실행 465)가 취소되고, 잡이 전부 skipped인 실행 466이 그 자리를 차지했다.** 제목·본문만 고친 `edited`는 모든 잡을 건너뛰도록 가드돼 있는데, 그 실행이 `ci-${{ github.ref }}` 같은 그룹에 있어 `cancel-in-progress`로 진짜 실행을 죽인 것이다. 결과적으로 커밋 `c2d4051`에 게이트 결과가 하나도 남지 않았다. concurrency 그룹 끝에 `meta`/`code` 구분자를 붙여 격리했다 |
| **조치 ⑤ 덮어쓸 이름 자체를 없앴다** | ④의 격리는 "진짜 실행이 죽는 것"만 막았다. **잡을 `if`로 건너뛰어도 그 이름의 체크 런은 `skipped`로 생성되고, 필수 체크는 skipped를 성공으로 취급한다** — 앞선 실행이 실패해 있었다면 본문 수정만으로 빨간 PR이 초록이 된다. master는 보호 브랜치라 실제 머지 위험이다. `edited`를 ci.yml에서 빼고 **`ci-retarget.yml`**로 분리했다. 거기서 건너뛰는 잡 이름은 `revalidate` 하나뿐이고 그 이름은 게이트가 쓰지 않으므로, 제목·본문 수정이 **덮어쓸 이름 자체가 존재하지 않는다.** ④의 concurrency 분기는 도달 불가가 되어 원래대로 되돌렸다 |
| **⑤-1 재발화 제약을 우회한 방법** | GITHUB_TOKEN으로 한 close/reopen·`workflow_dispatch`는 새 워크플로 실행을 만들지 않는다(PAT 필요) — 그래서 ④ 시점에는 분리를 포기했다. **`workflow_call`은 토큰 재발화가 아니라 같은 실행 안에서의 호출이라 이 제약을 받지 않는다.** ci.yml에 `workflow_call`을 열고 `ci-retarget.yml`이 base 변경 시에만 `secrets: inherit`으로 호출한다. github 컨텍스트는 호출자에서 상속되므로 PR head 기준으로 정상 실행되고, Phase 130이 `edited`를 넣은 이유(#92·#93)는 그대로 충족된다 |
| **⑤-2 감수한 것** | base 재지정 경로의 체크 이름은 `revalidate / ci` 형태가 되어 평소의 `ci`와 다르다. 필수 체크가 이름으로 걸려 있다면 재지정 뒤에는 푸시를 한 번 해야 머지된다. **초록으로 잘못 보이는 것보다 막히는 쪽이 안전하므로** 그대로 뒀다. 평상시(push·opened·synchronize·reopened) 이름은 하나도 바뀌지 않아 브랜치 보호 설정은 손댈 필요가 없다 |
| **검증** | 로컬에서 CI가 실행할 명령 그대로 `node --experimental-strip-types scripts/security-audit.ts` exit 0 · YAML 파싱으로 잡·의존 관계 확인 · 하네스 Doctor 0건. PR #115에서 **`security-audit`이 `ci`와 나란히 독립 체크로 등장**하고 8초에 통과함을 실측. ④는 GitHub 표현식의 느슨한 비교(`null == ''`)를 흉내낸 스크립트로 이벤트 6종에 대해 **"그룹이 `meta`인 경우 = 잡이 전부 건너뛰어지는 경우"가 정확히 일치**함을 확인했고, `7762f7b`의 CI가 도는 중에 일부러 본문을 고쳐 **메타 실행은 skipped로 끝나고 진짜 실행은 살아남는 것**을 실측했다. ⑤는 워크플로 6개를 파싱해 **"제목·본문 edited로 발화하는 워크플로의 잡 이름 ∩ 게이트 이름 = ∅"** 등 불변식 9개를 스크립트로 확인 |
| **남은 것** | ① vite 8 마이그레이션(청크 전략 재작성) — 별건 ② `deploy.yml`의 감사 스텝은 긴급 우회 경로 전용이라 그대로 뒀다 ③ 이 회차는 CI 구조만 봤다. 같은 "조용한 미실행"이 스케줄 워크플로(`functions-coverage.yml`)나 pre-push 훅에도 있는지는 확인하지 않았다 ④ base 재지정은 지난 두 번(#92·#93) 이후 발생하지 않아 `revalidate` 경로는 아직 실전 검증이 없다 — 다음 스택 PR에서 확인할 것 |

---

### Phase 137: 알림 패널이 하단 배너에 가려지던 문제 — z-50이 통하지 않은 이유 🪟 (커밋·PR)

> 2026-08-02, 사용자가 화면을 보다 발견했다. "원클릭 추천 예약 제목 레이어가 뒤로 가서 보이지 않아야 하는 것 아닌가?" 맞았다. **패널에 `z-50`을 줬는데도 `z-40` 배너가 그 위를 덮고 있었다** — 값을 잘못 준 게 아니라 값이 비교되는 무대가 달랐다.

| 항목 | 내용 |
|------|------|
| **증상** | 직원 대시보드에서 알림 패널을 열면 「원클릭 추천 예약」 제목·닫기 버튼이 패널 위로 올라온다 |
| **왜 `z-50`이 무력했나** | 알림 패널(`.notification-dropdown`, z-50)은 **헤더 안**에 있다. 그 헤더는 `sticky`+`z-30`으로도, `backdrop-blur-md`로도 **각각 stacking context를 만든다.** 그래서 패널의 z-50은 헤더 내부 순서일 뿐이고, **바깥에서 패널의 실질 높이는 헤더의 z-30**이다. 배너는 헤더 밖 최상위의 `fixed z-[40]`이라 40 > 30으로 이긴다 |
| **모바일도 같다** | 480px 미만에서 패널은 `position: fixed`로 바뀌지만, `backdrop-filter`는 **fixed 자손의 기준 상자까지 헤더로 바꾼다.** 뷰포트로 탈출하지 못해 여전히 헤더 안에 갇힌다 |
| **조치 ① 헤더를 배너와 모달 사이로** | 세 레이아웃(`Admin`·`SuperAdmin`·`Employee`)의 헤더가 모두 `z-30`으로 같았다 — 관리자 화면은 하단 고정 배너가 없어 드러나지 않았을 뿐 구조는 동일했다. **`z-[45]`**로 올려 고정 배너(40) 위, 모달(50) 아래에 둔다. 배너 z를 낮추는 선택지도 있었지만 **패널이 갇혀 있다는 원인은 그대로**라 다음 요소가 같은 함정에 빠진다 |
| **조치 ② 테스트가 잡아낸 두 번째 건** | 회귀 가드를 붙이자마자 실패했다 — 추천 예약 **접힘 상태의 플로팅 버튼이 **z-index 90**이라 헤더는 물론 **모달(50)까지 덮고 있었다.** 펼침 배너와 같은 층인 `z-40`으로 내렸다. 하단 내비(30) 위, 헤더(45)·모달(50) 아래다 |
| **가드가 한 번 틀렸다** | 첫 구현은 `ConsentGate.tsx` 안의 **하단 배너(z-40)와 관리자 모달(z-50)을 구분하지 못해** 모달까지 "헤더보다 낮아야 한다"고 요구했다. 모달은 헤더 위가 맞다. `inset-0`이면 모달, `bottom-*`이면 배너로 나눠 **양방향(배너보다 위 · 모달보다 아래)**으로 고정했다 |
| **검증** | 실제 Chromium으로 `sticky`+`backdrop-filter` 헤더 안의 z-50 패널과 바깥 z-40 배너를 재현해 **겹치는 좌표에서 `elementFromPoint`가 무엇을 반환하는지** 확인 — z-30이면 배너, z-45면 패널. 값 순서 가드는 **변형 4종(헤더 z-30 회귀 · 헤더 z-50 과상향 · 접힘 버튼 z-index 90 회귀 · 재동의 배너 z-index 46 상향)을 모두 실패로 잡음**을 확인했다. 전체 단위 테스트 1,044건 통과 |
| **곁가지 발견** | 이 회차를 기록하다 **문서 산문이 번들에 새는 것**을 봤다. Tailwind v4는 프로젝트를 자동으로 스캔하므로 구현이력에 적은 "z-index 46"·"z-index 90"을 클래스 문자열 형태로 적었더니 **쓰지도 않는 `.z-46`·`.z-90` 규칙이 프로덕션 CSS에 생성됐다.** 기능에는 영향이 없지만 문서가 번들을 키우는 경로라 값만 적는 표기로 바꿨다. 빌드 CSS의 z 유틸이 실제 사용분과 정확히 일치함을 재빌드로 확인 |
| **가드의 한계** | jsdom은 레이아웃·페인트가 없어 실제 stacking을 계산하지 못한다. 그래서 테스트는 **값의 순서만** 고정하고, 그 순서가 실제로 원하는 그림을 만드는지는 위 Chromium 확인에 기댄다. `?raw`로 소스를 읽는 이유는 프론트엔드 tsconfig에 Node 타입이 없어서다 |
| **남은 것** | ① 근본 해법은 패널을 `document.body`로 **포털**해 헤더의 stacking context를 아예 벗어나는 것이다. 바깥 클릭 감지가 포털 노드까지 봐야 해서 이번엔 하지 않았다 ② `z-[45]`는 배너(40)와 모달(50) 사이라는 뜻이 값 자체에 드러나지 않는다. 층 이름을 Tailwind 테마의 z 스케일로 정의하면 의도가 코드에 남는다 — 별건 |

---

### Phase 138: E2E 실패 증거가 스스로를 지우던 문제 — 그리고 부팅 청크 실패의 무한 로딩 해소 🔍 (커밋·PR)

> 2026-08-02, master CI가 E2E 한 건으로 빨개졌다. 재실행으로 통과해 플레이크로 확정했지만, **원인을 보려고 아티팩트를 열었더니 아무것도 업로드돼 있지 않았다.** 이 회차의 본론은 플레이크 자체가 아니라 **"정작 실패했을 때만 증거가 사라지는" 구조**였고, 파고들다 부팅 경로의 실제 결함을 하나 더 찾았다.

| 항목 | 내용 |
|------|------|
| **증상** | `Upload E2E failure artifacts` 스텝이 `No files were found with the provided path: test-results/`로 끝났다. 스텝 자체는 success라 실패로도 보이지 않는다 |
| **원인** | Playwright는 실행 시작 시 자기 `outputDir`를 지운다. 비인증 E2E와 인증 E2E가 **둘 다 기본값 `test-results/`를 쓰고 있었다.** 앞선 E2E가 실패해 스크린샷·trace를 남겨도, 뒤이어 도는 인증 E2E가 시작하면서 통째로 지운다 |
| **내가 만든 회귀다** | **Phase 136 이전에는 앞이 실패하면 뒤가 skipped라 지워질 일이 없었다.** 게이트 마스킹을 걷어내 둘이 각자 결론을 내게 만든 그 변경이, 증거 보존을 함께 깨뜨렸다. 옳은 변경이었지만 부작용을 못 봤다 |
| **조치** | `outputDir`를 `test-results/e2e`(비인증)와 `test-results/authed`(인증)로 분리. ci.yml 업로드 경로가 `test-results/` 전체라 워크플로는 손대지 않아도 그대로 수집된다 |
| **검증** | 일부러 실패하는 스펙으로 **실측**: 분리 전에는 1차 실행이 남긴 파일 5개가 2차 실행 뒤 1개로 사라졌고, 분리 후에는 5개가 그대로 살아남았다. 가드는 변형 4종(한쪽 outputDir 삭제 · 두 설정 동일 폴더 · 한쪽이 상위 폴더 · 업로드 경로 밖으로 이동)을 모두 실패로 잡는다 |
| **가드가 확인하는 것** | 두 `outputDir`이 존재하고, 서로 다르고, **한쪽이 다른 쪽을 포함하지 않고**, 둘 다 ci.yml 업로드 경로 안에 있을 것. 격리에 성공해도 업로드 경로 밖이면 결국 증거를 못 본다 |
| **조치 ② 부팅 청크 실패의 무한 로딩** | 플레이크 기전을 실험하다 별개의 실제 결함을 찾았다. 랜딩 청크(`LandingPage-*.js`)를 **두 번 연속 실패시키면 화면이 "로딩 중..."에서 10초를 넘겨 멈추고**, `Failed to fetch dynamically imported module`이 uncaught로 남는다. 1회 실패는 1.1초 더 걸릴 뿐 정상 복구된다 |
| **왜 ErrorBoundary가 못 잡나** | `main.tsx`가 `await import('./lightEntry')`/`await appEntryPreload`를 **`.catch()` 없이** 기다린다. 청크를 못 받으면 **React가 아예 마운트되지 않으므로** ErrorBoundary가 낄 자리가 없다. `lazyWithRetry`의 주석("두 번째도 실패하면 ErrorBoundary가 잡음")은 이 경로에는 해당하지 않는다 — 비인증 엔트리는 LandingPage를 lazy가 아니라 **정적 import**하기 때문이다. 처음 세운 `lazyWithRetry` 가설은 실험으로 기각됐다 |
| **②-1 어떻게 고쳤나** | `main.tsx`의 엔트리 로드를 `try/catch`로 감싸고, 실패하면 "앱을 불러오지 못했습니다 · 다시 시도" 화면을 띄운다. React가 없는 시점이라 로딩 화면과 같은 방식으로 직접 마크업을 넣는다. 비인증 사용자는 `appEntryPreload`를 await하지 않으므로 그 프라미스에도 no-op catch를 붙여 unhandledrejection을 막되, 원본은 그대로 두어 인증 경로의 await가 정상적으로 던지게 했다 |
| **②-2 검증** | `e2e/boot-failure.spec.ts` 신설 — 엔트리 청크를 막아 ① 재시도 화면이 뜨고 "로딩 중..."이 사라지는지 ② [다시 시도]로 회선 복구 후 정상 진입하는지 ③ **정상 회선에서는 실패 화면이 안 뜨는지(대조군)** 를 본다. `catch`를 제거한 빌드에서 앞의 두 건이 실패하고 대조군만 통과함을 실측해, 검사가 헛돌지 않음을 확인했다 |
| **남은 것** | 플레이크 자체의 확정 원인은 여전히 미상이다. 이제 증거가 남으므로 다음 발생 때 trace로 판정한다. 참고로 실험 중 관찰된 재로드는 `lazyWithRetry`가 아니라 **서비스 워커 `controllerchange` 핸들러**의 `window.location.reload()`였다 |

---

### Phase 139: 접속기록 조회 화면 — 점검 의무를 진 사람에게 점검 수단이 없었다 🔍📋 (커밋·PR)

> 2026-08-02, "접속기록은 누가 어떤 메뉴에서 확인하나"라는 질문에서 출발했다. 답을 찾아보니 **메뉴가 없었다.** 기록(Phase 123·132)과 Rules 권한은 완성돼 있었고, 고시 제16조 ②가 요구하는 월 1회 점검의 주체는 기관 관리자로 지정돼 있었는데, 그 관리자가 볼 화면만 Phase 3으로 남아 있었다 — **권한은 열렸고 의무는 부여됐는데 수단이 없는 상태**가 두 회차째 유지되고 있었다.

| 항목 | 내용 |
|------|------|
| **왜 이제 하나** | Phase 123이 "조회 화면은 Phase 3"으로 미뤘고 Phase 132는 기록 항목을 채우는 데 집중했다. 그 사이 Rules는 `isSuperAdmin() \|\| isOrgAdmin(resource.data.organizationId)`로 열려 있었다 — 즉 **기관 관리자는 볼 권리가 있는데 볼 방법이 없었다.** 고시가 요구하는 점검 주기(월 1회)에 비추면 화면 없는 상태는 점검을 사실상 불가능하게 만든다 |
| **선행 결함 — 스키마가 서버 값을 삼키고 있었다** | 화면을 붙이기 전에 드러난 것. `writeAuditEntry`(Phase 132)가 쓰는 수행업무 `export`·`read`와 대상 `export`·`orgDocument`가 **프론트 enum에 없었다.** Zod가 `.catch('update')`·`.catch('driveLog')`로 모르는 값을 대체하므로, 그대로 화면을 만들었다면 **반출 기록이 '운행일지 수정'으로 표시**된다. 저장은 정상이고 조회만 틀리는 유형이라 서버 테스트로는 절대 잡히지 않는다 |
| **왜 조용한 결함이 되나** | `.catch()`는 파싱 실패를 대체값으로 메꿔 화면 크래시를 막는 장치다. 그 장치가 **"모르는 값"과 "실제로 update인 값"을 구분 불가능하게** 만든다. Sentry에도 남지 않는다 — 파싱이 성공했기 때문이다. 그래서 `schemas/auditLog.test.ts`에 서버가 쓰는 값 전부를 고정하고, 알 수 없는 값이 update로 대체된다는 사실 자체도 테스트로 적었다. **서버 값이 또 늘면 이 테스트가 먼저 깨진다** |
| **조회 설계 — 유형을 4개로 묶었다** | 관리자에게 필요한 구분은 "누가 들어왔나 / 무엇이 바뀌었나 / 무엇이 나갔나"다. create·update·delete를 따로 고르는 UI는 점검에 도움이 되지 않으므로 전체·접속·변경·반출로 묶고, 수행업무는 `action in [...]` 하나로 던진다. **덕분에 유형이 늘어도 인덱스는 하나로 충분하다** — 유형별 등가 필터를 따로 만들면 인덱스가 유형 수만큼 늘어난다 |
| **인덱스** | `(organizationId, action, at 내림차순)` 추가. 유형 미선택(전체)은 Phase 123이 만든 `(organizationId, at 내림차순)`을 그대로 쓴다. 기간은 `at >=` 범위로 서버에서 자르고 커서(`startAfter`)로 50건씩 이어 읽는다 — 전량 조회를 열어 두면 기록이 쌓일수록 화면 진입 비용이 조용히 늘어난다 |
| **uid를 이름으로 바꾸는 지점** | 기록에는 최소수집 원칙에 따라 **uid만** 남는다(이름·이메일을 남기면 감사 로그가 또 하나의 개인정보 데이터셋이 된다). 그러나 uid는 점검하는 사람이 읽을 수 없다. **저장은 uid, 표시는 이름**으로 갈라 조회 시점에만 기관 구성원 목록으로 매핑한다. 구성원이 아닌 계정(서비스 운영자·탈퇴자)은 앞 6자만 보여준다 — 전체 uid는 화면에서 의미가 없고 줄만 길어진다. 이름 조회가 실패해도 기록 자체는 보여준다(점검을 막을 이유가 아니다) |
| **기록의 한계를 화면에 적었다** | ① 엑셀·PDF는 브라우저에서 생성돼 클라이언트가 부르지 않으면 기록이 남지 않는다 ② 삭제 행위자는 확정할 수 없어 '행위자 미확인'으로 남는다(남은 스탬프는 마지막 수정자이지 삭제자가 아니다 — Phase 132의 판단). **점검하는 사람이 한계를 모르면 "기록에 없으니 없었다"로 잘못 결론 낸다.** 화면에서 숨기지 않는 편이 맞다 |
| **Rules는 손대지 않았다** | 읽기 경계가 이미 점검 주체에 맞춰져 있다. 대신 **목록 조회(list) 경로를 테스트로 고정**했다 — 단건 `get`이 통과하는 것과 list가 통과하는 것은 별개다. 기관 필터가 빠진 쿼리는 타 기관 문서가 섞여 통째로 거부되는데, 그 경우 화면은 조용히 비고 원인은 드러나지 않는다. Rules 테스트 21 → **22건**으로 그 조건을 명시했다(필터를 지우면 테스트가 깨진다) |
| **배치** | 상단 메뉴가 아니라 설정 안에 뒀다. 월 1회 점검용 화면을 매일 쓰는 메뉴와 같은 층에 두면 상단이 어지러워지고, 실제로 찾는 시점은 "점검해야 할 때"다 |
| **검증** | lint · type-check · build(gzip 971.8/1050KB 예산 내) · 프론트 단위 1072 → **1080건**(도메인 9 · 훅 12 · 스키마 5 · 컴포넌트 8 신설) · Rules **22건**(에뮬레이터 실행) · 하네스 Doctor 0건 |
| **남은 것** | ① **인덱스 생성 시점** — 유형 필터는 배포 후 인덱스 빌드가 끝나야 동작한다(전체 유형은 즉시). 빌드 중에는 오류 문구가 뜬다 ② 정보주체(직원) 본인의 열람 요구는 앱 경로가 없고 처리방침 제12조(보호책임자) 문의로 처리된다 — 본인 기록 열람 화면이 필요한지는 별건 ③ 점검 사실 자체의 기록(누가 언제 점검했는가)은 남기지 않는다. 고시가 점검 대장을 요구하는지 확인한 뒤 판단 ④ superAdmin용 전체 기관 조회는 이번 범위 밖(`__system__` 기록 포함) |

---

### Phase 140: 접속기록 점검 실무에 맞춘 후속 — 기간 지정 · 내보내기 · 행위자 확정 📆📥

> 2026-08-03, Phase 139를 배포하고 화면을 실제로 열어 보자 세 가지가 어긋나 있었다. 셋 모두 **사용자가 화면을 보고 발견했다** — 기능은 동작했지만 점검이라는 용도에는 못 미쳤다. ① 동의 이력이 전부 '행위자 미확인' ② 보관은 1년인데 조회는 90일까지 ③ 점검 결과를 파일로 남길 방법이 없음.

| 항목 | 내용 |
|------|------|
| **① 왜 '행위자 미확인'이었나** | 변경 로그 트리거는 호출자를 볼 수 없어 문서에 심긴 `lastEditedByUid`에 의존한다. 그런데 동의 기록은 Rules가 클라이언트 쓰기를 막아 `acceptCurrentTerms`(Admin SDK)만 쓰고, **그 콜러블이 스탬프를 심지 않았다.** 서버는 uid를 알고 있었다 — 기관 문서에는 이미 `agreedByUid`로 남기고 있었고 `users` 문서에만 빠뜨렸다 |
| **왜 그냥 두면 안 되나** | 동의는 위탁 계약(약관 제9조) 성립의 입증 자료다. 행위자 없는 동의 기록은 "동의가 있었다"는 사실만 남고 "누가 했는가"가 빠진 절반짜리 증빙이다. 게다가 **접속기록은 수정 불가**라 나중에 채울 수 없다 — 빠진 채로 1년을 남는다(이미 쌓인 2건이 그렇게 됐다) |
| **어떻게 고쳤나** | users 배치에 `lastEditedByUid: uid` 한 줄. 값의 출처가 콜러블의 인증 컨텍스트라 클라이언트 경로에서 Rules가 강제하는 것과 같은 수준으로 위조 불가하다. `lastEditedByUid`는 `AUDITED_FIELDS`에 없어 변경 로그에 노이즈를 만들지 않는다(changedFields는 여전히 `consent` 하나) |
| **② 90일 한계** | 보관은 1년(고시 제16조)인데 기간 선택지가 7·30·90일이었다. **91일~1년 구간은 보관돼 있는데 점검 주체가 볼 수 없는 상태** — 볼 수 없는 보관은 점검 의무를 충족시키지 못한다. 365일을 추가했다. 서버 필터가 `at` 범위 조건이라 인덱스 변경 없이 범위만 넓어진다 |
| **③ 프리셋만으로는 월 1회 점검이 안 된다** | 프리셋은 오늘 기준 역산이라 "7월 한 달"을 잘라낼 수 없다. 시작일~종료일 직접 지정을 넣고 조회·내보내기가 같은 범위를 쓰게 했다. **양쪽이 다 채워졌을 때만 적용한다** — 한쪽만 입력된 중간 상태로 조회를 갈아치우면 날짜를 타이핑하는 동안 목록이 몇 번씩 바뀐다. 종료일은 23:59:59.999까지 포함(자정으로 자르면 마지막 날이 통째로 누락된다) |
| **내보내기 — 기간 전체를 다시 읽는다** | 화면 목록은 50건 단위 페이지네이션이다. 그것을 그대로 파일로 내보내면 "스크롤한 만큼"만 담겨 증빙이 되지 않는다. `getAuditLogsForExport`가 상한(5,000건)까지 한 번에 읽고, 상한에 걸리면 **잘렸다는 사실을 알린다** — 조용히 자르면 전량으로 오해한다 |
| **자기 참조를 받아들였다** | 접속기록 파일은 접속지 IP를 담으므로 이 내보내기 자체가 개인정보 반출이다. 서버 `DATASETS` 화이트리스트에 `auditLogs`를 추가해 **접속기록 반출이 다시 접속기록에 남게** 했다. 순환처럼 보이지만 점검 관점에서는 필요하다 — "점검 결과를 누가 가져갔는가"도 추적 대상이다. 화면에도 IP 포함 사실과 반출 기록이 남는다는 것을 적었다 |
| **라벨 표를 한 곳으로** | 화면과 엑셀이 각자 라벨 표를 갖고 있으면 한쪽만 갱신돼 **"화면은 반출인데 파일은 수정"**으로 어긋난다. 그 어긋남은 점검 결과를 잘못 읽게 만든다. `lib/auditLogLabels.ts`로 묶고 양쪽이 같은 표를 쓰게 했다(서버 값이 늘 때 갱신 지점도 하나가 된다) |
| **테스트가 결함 1건을 잡았다** | `since`/`until`을 한 `useMemo`로 묶었더니 **날짜 입력값이 바뀔 때마다 프리셋 Date가 새 객체로 만들어져** 조회 이펙트의 의존성이 바뀐 것으로 보이고 같은 기간을 다시 읽었다(적용 전 중간 상태에서도). "한쪽만 입력하면 재조회하지 않는다"를 먼저 테스트로 쓴 덕에 구현 직후 드러났다. 프리셋과 직접 지정을 따로 기억하도록 분리 |
| **검증** | lint · type-check(앱·functions) · 프론트 1080 → **1105건**(도메인 4 · 훅 9 · 엑셀 6 · 화면 4 신설) · Functions **34건**(반출 화이트리스트 · 스탬프) · 빌드(gzip 973.6/1050KB) |
| **남은 것** | ① 중복 인덱스 `auditLogs (action, organizationId, at↓)` 정리 — 유형 필터 동작 확인 후 콘솔에서 삭제 ② 점검 사실 자체의 기록(누가 언제 점검했는가)은 여전히 없다 ③ 정보주체 본인 열람 화면 미제공(처리방침 제12조 경로) ④ PDF 출력은 없다 — 점검 보관 용도로 엑셀이 맞다고 보고 넣지 않았다 |

---

### Phase 141: 배포 파이프라인·인덱스 정리 후속 — red 해소 · 중복 인덱스 · '행위자 미확인'의 사유 확정 🧹🔍

> 2026-08-03, Phase 140을 master에 올리자 CI가 빨개져 배포가 멈췄다. 접근성 E2E 한 건이었고, 원인은 **테스트가 아니라 단언 방식**이었다. 이어서 Phase 140이 "남은 것"으로 걸어 둔 중복 인덱스를 정리했고, 그 과정에서 **로컬 저장소가 3일 낡아 있었던 것**이 하마터면 TTL 정책을 지울 뻔했다. 마지막으로 사용자가 화면에서 물은 "왜 '행위자 미확인'인가"에 답을 확정했다.

| 항목 | 내용 |
|------|------|
| **① CI red — 단언이 문제였다** | `e2e/accessibility-advanced.spec.ts`의 "네비게이션에 aria-label이 있다"가 실패. **nav이 사라진 회귀가 아니다** — 바로 앞 줄 `toBeAttached()`가 통과했으므로 그 순간 요소는 존재했다. 다음 줄 `count()`가 0을 읽었다: `count()`는 **재시도 없는 일회성 읽기**라 랜딩 진입 직후 라우팅 판정으로 트리가 교체되는 몇 ms와 겹치면 그 순간을 그대로 실패로 굳힌다 |
| **플레이크로 넘기지 않은 근거** | 초기 시도와 재시도 2회가 **3/3 같은 지점에서** 깨졌다. 반면 로컬에서 CI와 같은 조건(빌드 산출물 + `vite preview`, 재시도 0)으로는 72건 전부 통과 — 타이밍이 조금 달라지면 언제든 다시 깨지는 구조라는 뜻이다. 웹 우선 단언 `not.toHaveCount(0)`으로 바꿨다(검증 의도 불변, 판정만 재시도형). 같은 함정을 갖고 있던 `aria-live` 테스트도 함께(고정 대기 2초 + 일회성 count) |
| **로컬 재현 수단** | 컨테이너 Chromium(1194)과 Playwright 1.62가 기대하는 headless shell(1234)이 어긋나 로컬 실행이 막혔다. 저장소 설정을 건드리지 않고 **스크래치패드의 임시 설정**으로 `executablePath`만 덮어 실행했다 — 저장소에 넣으면 CI가 잘못된 브라우저를 쓰게 된다 |
| **② 중복 인덱스 제거** | `auditLogs (action, organizationId, at↓)`는 콘솔 오류 링크로 생성된 것으로, 코드가 쓰는 `(organizationId, action, at↓)`와 **등가 필터 순서만 다른 사본**이었다. auditLogs는 로그인·수정마다 쓰기가 발생해 중복 인덱스가 쓰기·저장 비용을 그대로 먹는다 |
| **삭제해도 되는 근거를 코드베이스에서 찾았다** | 추측 대신 기존 동작으로 확인. `getDriveLogs`는 `where('driverUid')`를 `where('organizationId')`보다 **먼저** push하는데, 그 쿼리를 만족시킬 인덱스는 `(organizationId, driverUid, timestamp↓)` 하나뿐이다(`(driverUid, timestamp↓)`에는 organizationId가 없어 등가 조건을 만족시킬 수 없다). 관리자 운행일지의 운전자 필터가 프로덕션에서 동작하므로 **인덱스의 등가 필드 순서는 제약 순서와 무관하다** |
| **실증** | 삭제 후 접속기록의 [접속]·[변경]·[반출·열람] 필터가 정상 동작했다 — 근거가 맞았음을 프로덕션에서 확인. 프로덕션 인덱스 38 → **37개**로 파일과 1:1 일치 |
| **③ 근접 사고 — 로컬이 3일 낡아 있었다** | 삭제는 로컬 `firestore.indexes.json`을 기준으로 하는데, 운영자 로컬은 `9aed653`(7/31) 상태였다. **pull 없이 `--force`를 실행했다면** 그 옛 파일 기준으로 정리돼 인덱스 11건과 **TTL 정책 3건(접속기록 1년 자동 파기 포함)**이 지워졌을 것이다. 사전 조건으로 `git pull` → `git log -1` 확인을 먼저 시켰고, 130개 파일이 내려온 것으로 낡음이 드러났다 |
| **`--force` 대신 대화형 확인** | firebase-tools는 대화형 터미널에서 `--force`가 없으면 **삭제 목록을 보여주고 y/N을 묻는다**(비대화형에서는 안내만 하고 기본값 '아니오'). 목록에 1건만 뜨는 것을 눈으로 확인하고 `y` — 눈으로 보고 지우는 절차가 `--force`보다 안전하다. 결과: `Deleting 1 indexes` |
| **④ '행위자 미확인'의 사유 확정** | 화면의 동의 이력 2건(8/1 15:25, 8/2 14:02)은 스탬프 수정(`de812db`)이 **배포되기 전**에 기록된 것이다. 표시가 둘로 나뉘는 이유도 필드가 다르다 — `actorUid: null` → "알 수 없음", `actorSource:'unknown'` → "(행위자 미확인)". 즉 기록 자체가 "알아낼 방법이 없었다"를 밝히고 있다 |
| **소급하지 않는다는 판단** | 서버로 값을 채워 넣는 것은 기술적으로 가능하지만 하지 않는다. Rules가 클라이언트 쓰기를 전면 차단한 이유가 위조·인멸 방지인데, **"나중에 고칠 수 있는 감사 기록"은 감사 기록이 아니다.** 빠진 채로 남기고 화면이 그 사실을 밝히는 편이 정직하다 |
| **앞으로의 표시 규칙(확인 결과)** | 재동의(`acceptCurrentTerms`) → 이름·부연 없음(stamp) / 신규 가입(`joinOrganization`) → 이름 + "문서 기록으로 추정"(`auditUserCreated`가 문서 ID를 행위자로 쓴다, `restoredAt`으로 관리자 복원과 구분) / 운행일지·직원 정보 수정 → 이름(클라이언트 스탬프 + Rules 강제) / **삭제 → 계속 미확인**(남은 스탬프는 마지막 수정자이지 삭제자가 아니다 — Phase 132의 판단) |
| **검증** | lint · 비인증 E2E 72건(재시도 0, 로컬 Chromium 1194) · CI·CodeQL·보안감사·functions-tests 전체 통과 · Deploy 성공(`ba6943f` 09:31 → Functions 2개 포함, `167a725` 10:04 → 인덱스 변경 0건) |
| **남은 것** | ① 점검 사실 자체의 기록(누가 언제 점검했는가)은 여전히 없다 — 고시가 점검 대장을 요구하는지 확인 후 판단 ② 정보주체 본인 열람 화면 미제공(처리방침 제12조 경로) ③ 트랙B 파일이 190KB 근처 — 200KB 임계 전에 `트랙B_Phase103-141.md`로 한 번 리네임하고 새 `…부터` 파일을 열어야 한다 ④ `fnm` 셸 프로필 미설정(운영자 로컬) — 인덱스 배포에는 무해하나 빌드·Functions 배포 시엔 Node 22가 필요하다 |
