...
| **설명** | 개인정보를 담는 컬렉션(`driveLogs`·`users`)의 생성·수정·삭제를 `auditLogs`에 기록한다. 고시 「개인정보의 안전성 확보조치 기준」 제16조(접속기록의 보관 및 점검) 대응. 수정은 **변경된 필드명만** 남기고 값은 남기지 않는다(로그가 개인정보 스냅샷이 되는 순환 방지). |
| **기록 범위** | **개인정보 필드와 접근 권한 필드만**(`AUDITED_FIELDS` 화이트리스트). km 연쇄 동기화·다크모드 토글·FCM 토큰 회전 등은 기록하지 않는다 — 시스템 파급이 사람의 편집을 묻어 제16조 ②의 점검을 불가능하게 만들고, 클라이언트가 임의 필드명을 주입해 로그를 오염시키는 경로도 막는다. |
| **인증** | 시스템 자동 실행 (Firestore 트리거, Admin SDK) |
| **보관** | `expiresAt = at + 365일`. **TTL 정책을 콘솔에서 설정해야 자동 삭제된다** → [MONITORING_GUIDE](MONITORING_GUIDE.md) |
| **유실 대책** | `retry: true` + 멱등 문서 ID(`대상타입_대상ID_이벤트ID`). 실패는 ERROR 로그(Sentry)를 남기고 **다시 throw해** 재전달을 받는다. 삼키면 쿼터 초과·일시 장애에 법정 기록이 조용히 사라진다. 트리거 실패는 원본 문서 쓰기를 되돌리지 않는다(트리거는 커밋 이후 실행). |
| **한계** | Firestore 트리거는 호출자를 알 수 없다. 생성은 문서의 `createdByUid`로 추정하고(사용자 문서는 `restoredAt`으로 본인 가입/관리자 복원을 가름), 수정·삭제는 `actorUid: null` + `actorSource: 'unknown'`이다. 행위자 식별은 열람 로그·세션 기록과 함께 후속 단계에서 처리한다. 기관 미소속 계정(주로 superAdmin)은 `organizationId: '__system__'`으로 남겨 superAdmin만 조회한다. |
