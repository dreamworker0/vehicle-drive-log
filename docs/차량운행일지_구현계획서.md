...
| Phase | 일자 | 요약 |
|------|------|------|
| 95 | 2026-07-11 | 보안 개선 — PII 로그 마스킹(`utils/mask.ts` 신설, 알림톡 전화번호+이름 3곳·이메일 11곳), 기관 신청 MIME 화이트리스트(jpeg/png/webp/pdf, 개선계획 작업 3), Rate Limit `failMode` 옵션 + 고위험 5곳(OCR·AI·공개 접수) fail-closed(작업 2, OCR 일일 쿼터 항상 closed), 실크리덴셜 4종 defineSecret 이관(`core/params.ts`, 소비 함수 12곳 secrets 바인딩). App Check 2·3배치는 Authentication 미검증 30% 원인 규명까지 보류. Functions 31 suites/284 통과. defineSecret 커밋(`7c15fb6`)은 시크릿 등록 + .env 정리 전 푸시 금지 |
