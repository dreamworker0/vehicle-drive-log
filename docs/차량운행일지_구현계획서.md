...
> **최종 업데이트**: 2026-07-20 | **전체 진행률**: Phase 1~58 완료 ✅ (테스트 신뢰성 강화 및 거대 훅 분해), 운영 로그 Phase 108까지 기록(65까지 배포 완료, 66~92 CI 배포, 93~96 코드 완료·배포 대기, 97~108 CI 배포) | 서비스 운영 중 (210개+ 기관)
...
두 개의 독립 번호 트랙이 있다 — **트랙 A**(초기 상세 이력 Phase 1~58) / **트랙 B**(운영 고도화 로그 Phase 49~107). 시기가 겹쳐 단일 연속 번호로 합치지 않고 트랙을 분리 표기한다.
...
| 106 | 2026-07-19 | 어시스턴트 수정 되묻기 맥락 기억 + 생성 시작만 입력 시 종료 자동 — 프로덕션 실사용에서 발견. **A** 수정 N건 되묻기 후 "EV52465를 적용해줘"가 맥락 잃던 문제: 새 값+후보를 `assistantConversations`(`kind='modify'`·10분 TTL)에 저장, `resolveModifySelection`(서수·차량명·시작시각 매칭, LLM 재파싱 없음)으로 다음 메시지를 후보 선택 해석(1건 확정/여러 건 재되묻기/0건 주제전환). `buildModifyResult` 공유·대화 상태 로더 create/modify 판별 확장(하위 호환). **B** 생성 시 시작만 있고 종료 없으면 +1시간 자동(앱 `calcEndTime(start,0)` 동일·23:59 상한, 되묻지 않음). 확인 버튼이 안전망. 47 suites/476 통과(신규 7). 커밋 `f4c3fbe`. 목적지+TMAP 자동 종료계산(C)은 후속 |
