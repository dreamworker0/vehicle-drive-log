...
| Phase | 일자 | 요약 |
|------|------|------|
| 87 | 2026-07-06 | firestore 도메인 데이터 접근 테스트 신설 — `src/lib/firestore/`의 CRUD 함수에 테스트가 없던 공백(테스트 6.0/10, 개선계획서 지속 트랙 1번)을 vehicles(10건)·users(18건)부터 메꿈. org 격리 `where` 단언 필수화, `.withConverter` 체이닝 스텁 등 재사용 가능한 firebase/firestore mock 하네스 확립. reservations·driveLogs(트랜잭션·오프라인 큐)는 후속 신중 트랙으로 분리. Opus 계획 + Fable 5 실행 분업 첫 작업 |
