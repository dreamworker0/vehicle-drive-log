...
| Phase | 일자 | 요약 |
|------|------|------|
| 129 | 2026-07-31 | 다일·반복 예약 수정이 항상 실패하던 문제 — Phase 128 리뷰가 "범위 밖 관찰"로 남긴 의심을 에뮬레이터로 재현했더니 **살아 있는 버그**였다. 그룹 수정은 "기존 그룹 삭제 → 재생성" 구조라 삭제 권한이 필요한데 `firestore.rules`의 reservations `delete`가 `isSuperAdmin()` 한정이라, superAdmin 외 **전원**(직원·기관 관리자)이 다일·반복 예약을 수정하면 `PERMISSION_DENIED: false for 'delete' @ L192`로 실패했다. 사용자에게는 Firebase 원문 `Missing or insufficient permissions.` 영문 토스트가 뜨고 원본 그룹은 남았다. 도달 경로는 전부 살아 있다(`onEdit={handleEdit}` → `setEditingGroupId(res.groupId)` → `deleteReservationGroup()` → `batch.delete()`). 규칙이 Phase 30 무렵부터라 그룹 수정은 사실상 도입 이후 계속 이 상태였고, 다일·반복 예약 **수정**이 드문 조작이라 묻혀 있었다. 조치는 `allow delete`를 **update 규칙과 같은 범위**(소유자 본인 + 소속 기관 관리자)로 한정 완화 — 소유자는 어차피 자기 예약을 취소·수정할 수 있었으므로 새로 열리는 권한이 아니고, 타 기관·같은 기관 타인 예약 삭제는 계속 차단. **양방향 뮤테이션 실측**: 규칙 하나에 허용·차단이 함께 걸려 한 방향만 재면 반쪽이라, ① `isSuperAdmin()`만 → 허용 케이스 실패(재현 그 자체) ② `belongsToMyOrg()`만 → 차단 케이스 실패(`Expected request to fail, but it succeeded`) 양쪽 확인. Rules 테스트에 reservations `delete` 케이스가 **아예 없어** 회귀로도 안 잡히던 것을 4방향(소유자·기관 관리자 허용 / 같은 기관 타인·타 기관 차단)으로 신설, Firestore Rules 16 → **17건**. **남는 것**: 그룹 수정의 "삭제 → 재생성"이 트랜잭션이 아니라, 재생성 실패 시 예약이 통째로 사라질 수 있다 — 지금까진 삭제에서 막혀 데이터가 보존됐던 셈이라 이 수정으로 위험이 드러난다. 근본 해결은 그룹 수정 전체를 콜러블(Admin SDK 트랜잭션)로 이전하는 것이고 작업량이 커 별건 |
