...
> **최종 업데이트**: 2026-04-24 12:41 | **전체 진행률**: Phase 1~54 완료 ✅ (다크모드 기본값 적용, 팝업 로그인 인증 안정화, 사업자번호 82 자동승인, 시스템 최적화 반영) | 서비스 운영 중 (150개 기관)
...
| **보안** | Firebase App Check (reCAPTCHA Enterprise) |
...
사용자의 이전 예약 기록을 통계적으로 분석해 다음 일정을 예측하고 원클릭 추천 배너를 표시합니다.

1. **데이터 추출**: 대상 사용자의 최근 예약 30건을 조회합니다.
2. **패턴 검증**: `요일 + 시간 + 차량 + 목적지` 조합 빈도를 계산하여, 3회 이상 동일 패턴이 발견되면 해당 요일과 시각을 다음 예약일로 예측합니다.
3. **가용성 확인 및 차선책 (Smart Alternative)**: 
   - 사용자가 추천 시간에 다른 예약을 이미 잡은 경우 최대 10주까지 확인하여 일정을 미룹니다.
   - 추천 1순위 차량이 이미 예약 마감된 경우, **자신이 평소 자주 탔던 다른 가용 차량 중 가장 많이 이용한 차량을 자동으로 찾아** 대체 추천합니다 (스마트 대체 로직).
4. **빠른 예약**: **오늘 대시보드 최하단에 단일 박스 형태의 목록(리스트) UI로 추천 내역 전체가 노출**되며, 우측의 '예약' 버튼 클릭 시 해당 정보(`prefillPattern`)가 겹치지 않는 가장 빠른 날짜로 예약 폼에 진입합니다.
...
현재의 공용 서비스 계정(Service Account) 방식에서 개별 사용자의 캘린더에 직접 연동하는 방식으로 설계 확장을 계획 중입니다.

*   **권한 동의**: 앱 내에서 "Google 캘린더 개인 연동" 클릭 시 OAuth 2.0 Consent Screen 호출
*   **토큰 보관**: 발급된 Access Token과 Refresh Token을 Firestore의 해당 사용자(`users/{uid}`) 프로필에 안전하게 저장
*   **이벤트 생성 주체**: 해당 사용자가 예약 생성 시 서비스 계정 대신 본인의 Access Token을 사용하여 개인 캘린더에 직접 이벤트 생성
*   **오프라인 갱신**: Access Token 만료 시, 시스템이 Refresh Token을 이용해 자동으로 Access Token 재발급 수행
*   **정책 심사**: 민감한 권한(Google Calendar API Scope) 사용에 대해 앱 배포 전 Google Cloud Console에서 OAuth 동의 화면 심사 필수
