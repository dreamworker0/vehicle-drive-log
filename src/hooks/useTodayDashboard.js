...
export default function useTodayDashboard() {
    ...
    const [incompleteAlerts, setIncompleteAlerts] = useState([]);
    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            try {
                ...
                setIncompleteAlerts(
                    incomplete.map(res => ({ type: 'reservation', ...res }))
                );
            } catch (err) {
    ...
    const recommendedVehicle = useMemo(() => {
        ...
        return candidates.sort((a, b) => b.minutesUntilNext - a.minutesUntilNext)[0];
    }, [vehicles, todayReservations]);

    const handleStartDrive = async (reservation) => {
    ...
    const handleStartNavigation = async (reservation, app = 'tmap') => {
        setStartingId(reservation.id);
        try {
            const now = new Date();
            const actualStartTime = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
    ...
    const handleCancelWeekReservation = async (reservation) => {
