...
export default function useMonthlyReport() {
    ...
    const filteredLogs = useMemo(() =>
        logs.filter(l => {
            const d = l.date || l.timestamp?.toDate?.()?.toISOString?.()?.slice(0, 10) || '';
            if (!d) return false;
    ...
    const prevPeriodLogs = useMemo(() => {
        ...
        const daysDiff = Math.ceil((e - s) / (1000 * 60 * 60 * 24));
    ...
    const stats = useMemo(() => {
        ...
        const s = new Date(startDate);
        const e = new Date(endDate);
        const daySpan = Math.max(1, Math.ceil((e - s) / (1000 * 60 * 60 * 24)) + 1);
    ...
    const dailyTrendData = useMemo(() => {
        return Object.entries(stats.byDate || {})
            ...
            .map(([date, data]) => ({ date: date.slice(5), count: data.count, distance: data.distance }));
    }, [stats.byDate]);
