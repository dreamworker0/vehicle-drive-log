 ...
 * @param {Object} options - 추가 옵션 { isElectric, isQuickDrive }
 * @returns {{ valid: boolean, message: string|null }}
 */
export function validateDriveLogForm(form, { isElectric = false, isQuickDrive = false } = {}) {
    if (!form.vehicleId) {
 ...
 * @param {string} driveDate - YYYY-MM-DD 형식
 * @param {string} endTime - HH:MM 형식
 * @param {string} startTime - HH:MM 형식
 * @returns {Date}
 */
export function buildDriveTimestamp(driveDate, endTime, startTime) {
    if (driveDate && driveDate !== todayStr()) {
        // 소급 입력: 선택한 날짜 + endTime 또는 현재 시각
 ...
 * @param {Object} form - 폼 데이터
 * @param {Object} context - { orgId, user, userData, selectedVehicle, selectedPassengers, isRetroactive }
 * @returns {Object} logData
 */
export function buildLogData(form, { orgId, user, userData, selectedVehicle, selectedPassengers, isRetroactive }) {
    const startKm = parseInt(form.startKm);
    const endKm = parseInt(form.endKm);
    ...
    return {
        ...
        passengerCount: selectedPassengers.length + 1,
