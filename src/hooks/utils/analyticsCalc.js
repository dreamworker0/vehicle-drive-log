 ...
 */
export const getLogDate = (l) =>
    l.date || l.timestamp?.toDate?.()?.toISOString?.()?.slice(0, 10) || '';
 ...
 */
export function getWorkdaysInMonth(yearMonth) {
...
/**
 * 월별 운행 추이를 계산한다.
 * @param {Array} logs - 필터링된 운행 로그 목록
 * @param {Array} monthKeys - 월 키 목록 ['2025-09', ...]
 * @returns {Array} [{ month, count, distance, fuelCost, label }]
 */
export function calcMonthlyTrend(logs, monthKeys) {
    const map = {};
    ...
    logs.forEach(l => {
        ...
        if (!map[mk]) return;
        map[mk].count++;
        map[mk].distance += (l.endKm - l.startKm) || 0;
 ...
 * @param {Array} logs
 * @returns {{ grid: number[][], items: Array, maxCount: number }}
 */
export function calcHeatmapData(logs) {
...
export function detectAnomalies(logs) {
    ...
    logs.forEach(l => {
        ...
        dailyDist[key].distance += (l.endKm - l.startKm) || 0;
    });
