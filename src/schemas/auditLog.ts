 ...
 *
 * ⚠️ 서버가 쓰는 값을 하나라도 빠뜨리면 안 된다 — 아래에서 `.catch('update')`로
 ...
 * - `stamp`: 클라이언트가 심은 `lastEditedByUid`. Rules가 `request.auth.uid`와의 일치를
 *   강제하므로 타인 명의로는 위조할 수 없다 — 네 출처 중 신뢰 수준이 가장 높다.
 * - `auth`: 인증 토큰에서 직접 확인. 콜러블이 `request.auth.uid`로 받으므로 위조 불가하다.
 *   (세션 기록 전용 — 트리거는 인증 컨텍스트를 볼 수 없다.)
 * - `document`: 문서 필드(createdByUid 등)에서 추정.
 * - `unknown`: 트리거가 알 수 없음 (서버 쓰기·삭제 등).
 */
export const auditActorSourceSchema = z.enum(['stamp', 'auth', 'document', 'unknown']);

/**
 * 접속기록/변경 로그 스키마 — 이 파일이 문서 모양의 단일 원본이다.
 * `types/auditLog.ts`의 `AuditLog`은 여기서 `z.infer`로 파생된다.
 *
 * at·expiresAt은 트리거가 **항상** 쓰므로 필수로 둔다.
 */
