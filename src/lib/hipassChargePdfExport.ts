...
export function downloadHipassChargePdf(
    ...
    // 페이지 분할
    const pages: PdfHipassChargeEntry[][] = [];
 ...
 * 시각 문자열 추출
 */
function getTimeStr(entry: PdfHipassChargeEntry): string {
...
function buildRow(rec: PdfHipassChargeEntry, idx: number, pageIdx: number) {
    return `
        <tr>
 ...
 * 합계 행
 */
function buildTotalRow(pageRows: PdfHipassChargeEntry[]) {
    ...
    return `
        <div class="page">
            <div class="header-area">
                <h1 class="title">하 이 패 스 충 전 기 록</h1>
                ${approvalHtml}
            </div>
            <div class="info-row">
                <div class="info-left">
            ...
            <table class="log-table">
                <thead>
                    <tr>
                        ...
                        <th class="col-charger">충전자</th>
                        <th class="col-card">하이패스 카드</th>
                        <th class="col-amount">충전금액(원)</th>
                        <th class="col-before">충전전잔액</th>
                        <th class="col-after">충전후잔액</th>
                    </tr>
                </thead>
                <tbody>
                    ${rowsHtml}
 ...
 */
function getHipassChargePdfStyles() {
    return `
        @page {
            size: A4 landscape;
            margin: 12mm 10mm;
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .col-charger { width: 60px; }
        .col-card { width: 100px; }
        .col-amount { width: 80px; }
        .col-before { width: 80px; }
        .col-after { width: 80px; }
 ...
 * 전체 HTML 문서 생성
 */
function buildPdfHtml(pages: PdfHipassChargeEntry[][], options: { orgName: string; approvalLine: ApprovalEntry[] }) {
...
<head>
    <meta charset="UTF-8">
    <title>하이패스 충전 기록 - ${options.orgName}</title>
    <style>${getHipassChargePdfStyles()}</style>
</head>
<body>
    ${pagesHtml}
</body>
