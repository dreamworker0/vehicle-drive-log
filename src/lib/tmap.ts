...
export const getMultiRoute = async (origin: string, destinationText: string, { carType = '0' } = {}) => {
    ...
    const coords = await Promise.all(allAddresses.map((addr: string) => geocode(addr)));
    if (coords.some(c => !c)) return null;

    // 구간별 경로 탐색: origin→dest1, dest1→dest2, ..., destN→origin
    const segments: { from: string; to: string; distance: number; duration: number; tollFee: number }[] = [];
...
export const getNaverMapDeeplink = async (destination: string) => {
    ...
    try {
        const coords = await Promise.all(dests.map((d: string) => geocode(d)));
...
/**
 * 목적지 문자열로 경로 정보 조회
 * useReservationCalendar에서 사용
 * @param {string} destination - 목적지 주소/장소명
...
export const getRouteInfo = async (destination: string) => {
    try {
        ...
        // 기본 좌표 (0,0) → 실제 출발지가 없으므로 거리/시간만 참고용
