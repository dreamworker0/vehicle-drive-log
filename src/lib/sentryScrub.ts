...
function scrubValue(value: unknown, keySafe: boolean, depth: number, path: Set<object>): unknown {
    ...
    try {
        ...
        for (const key of Object.keys(value as Record<string, unknown>).slice(0, MAX_OBJECT_KEYS)) {
...
/** console breadcrumb의 인자 배열처럼 키가 없는 값 목록을 거른다. */
export function scrubValues(values: unknown[]): unknown[] {
    const path = new Set<object>();
    return values
        .slice(0, MAX_ARRAY_ITEMS)
        .map(value => scrubEntry(value, false, 0, path));
}
