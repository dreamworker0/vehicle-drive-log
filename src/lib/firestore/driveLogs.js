...
// 운행일지 생성 (중복 방지 체크 포함)
export const createDriveLog = async (data) => {
...
export const getLastVehicleEndKm = async (orgId, vehicleId) => {
    const q = query(
        collection(db, 'driveLogs'),
...
export const getVehicleEndKmBefore = async (orgId, vehicleId, beforeDate) => {
    const q = query(
        collection(db, 'driveLogs'),
        where('organizationId', '==', orgId),
        where('vehicleId', '==', vehicleId),
 ...
 */
export const syncNextLogStartKm = async (orgId, vehicleId, afterDate, newStartKm) => {
    const q = query(
        collection(db, 'driveLogs'),
        where('organizationId', '==', orgId),
        where('vehicleId', '==', vehicleId),
...
export const getDriveLogs = async (orgId, filters = {}) => {
    ...
    if (filters.startAfter) {
        ...
        constraints.splice(3, 0, startAfterFn(filters.startAfter));
    }
...
// 내 운행일지 목록 조회
export const getMyDriveLogs = async (orgId, uid, limitCount = 30) => {
    const q = query(
        collection(db, 'driveLogs'),
        where('organizationId', '==', orgId),
        where('driverUid', '==', uid),
...
// 운행일지 수정
export const updateDriveLog = async (logId, data) => {
...
// 차량별 운행일지 조회
export const getVehicleDriveLogs = async (vehicleId) => {
    const q = query(
        collection(db, 'driveLogs'),
        where('vehicleId', '==', vehicleId)
    );
...
export const cleanupDuplicateLogs = async (organizationId, { dryRun = true } = {}) => {
