...
// 정비 기록 목록 조회
export const getMaintenanceRecords = async (orgId, vehicleId = null) => {
    let q;
    if (vehicleId) {
        q = query(
...
export const deleteMaintenanceRecord = async (recordId, vehicleId = null) => {
...
// 차량 정비 차단 수동 해제
export const clearVehicleMaintenanceBlock = async (vehicleId) => {
    await updateDoc(doc(db, 'vehicles', vehicleId), {
        maintenance: null,
    });
};
...
export const cancelVehicleReservations = async (orgId, vehicleId, vehicleName, startDate, endDate, reason) => {
    const q = query(
