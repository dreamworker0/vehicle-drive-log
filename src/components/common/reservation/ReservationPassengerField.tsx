/**
 * ReservationPassengerField — 동승자 입력 (예약 폼 · 바로 운행 공용)
 *
 * 운행일지와 **같은** 입력 컴포넌트(PassengerSection)를 쓴다. 두 화면이 갈라지면
 * 기관이 정한 입력 방식(목록·검색·인원 숫자)이 한쪽에서만 지켜지는 일이 생긴다.
 *
 * 이 컴포넌트가 하는 일은 두 가지뿐이다.
 *  (1) 폼 상태(PassengerFormValues)와 PassengerSection의 입출력을 잇는다.
 *  (2) 기본으로 접어 둔다 — 예약 폼도 바로 운행 폼도 이미 차량·목적지·목적으로 빽빽하고,
 *      동승자는 선택 입력이라 펼쳐 두면 매번 지나쳐야 하는 벽이 된다.
 *
 * 폼 상태의 모양이 화면마다 달라(예약은 날짜·반복까지, 바로 운행은 차량·목적지뿐)
 * `setForm`을 통째로 받지 않고 **바뀐 동승자 필드만** onChange로 올려보낸다.
 */
...
interface Props {
    ...
    /** 바뀐 동승자 필드만 담아 올려보낸다 */
    onChange: (patch: PassengerFormValues) => void;
    members: UserDoc[];
    allowList?: boolean;
    allowSearch?: boolean;
    allowCount?: boolean;
    /** 화면별 안내 문구 — 예약은 '예정 인원', 바로 운행은 '지금 함께 타는 인원' */
    hint?: ReactNode;
}

export default memo(function ReservationPassengerField({
    values, onChange, members, allowList = true, allowSearch = true, allowCount = true, hint,
}: Props) {
    const selectedPassengers = useMemo(
        () => members.filter(m => (values.passengerUids || []).includes(m.id)),
        [members, values.passengerUids],
    );
    ...
    const togglePassenger = (member: UserDoc) => {
        const current = values.passengerUids || [];
        onChange({
            ...
            {isOpen && (
                <div className="mt-1 animate-fade-in">
                    <PassengerSection
                        ...
                        setExternalPassengerCount={count => onChange({ passengerCount: count })}
                        setExternalPassengerNames={names => onChange({ passengerExternalNames: names })}
                    />
                    {hint && (
