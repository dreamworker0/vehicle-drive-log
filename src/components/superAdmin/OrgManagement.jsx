...
import { getApprovedOrganizations, deleteOrganization, getDeletedOrganizations, restoreOrganization, permanentDeleteOrganization, getOrganizationMembers, updateUser, leaveOrganization } from '../../lib/firestore';
...
export default function OrgManagement() {
    ...
    const fetchOrganizations = async () => {
        ...
        try {
            const [orgs, deleted] = await Promise.all([
                ...
                getDeletedOrganizations().catch((err) => {
                    console.warn('삭제된 기관 목록 로드 실패 (인덱스 빌드 중일 수 있음):', err);
                    return [];
                }),
            ]);
            setOrganizations(orgs);
            setDeletedOrgs(deleted);
    ...
    const handleRoleChange = async (member, orgId, newRole) => {
