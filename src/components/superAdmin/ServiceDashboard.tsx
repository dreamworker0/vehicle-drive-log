        ...
        <div className="animate-fade-in space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-col items-end gap-1">
                    ...
                    {/* ── 갱신 비용 안내 ──
                        이 버튼 한 번이 전 기관 데이터를 다시 스캔한다. 실측(2026-09-09)으로
                        클릭당 약 1.3~1.6만 read이고 하루 무료 한도(5만)의 3분의 1 가까이다.
                        같은 기간 실제 사용은 30일에 24회(약 0.8회/일)였으므로, 야간 자동 갱신이
                        있다는 사실만 알려도 불필요한 클릭이 줄어든다 — 운영자가 한 명이라
                        화면에 적어 두는 것이 가장 값싼 대책이다(쿨다운은 이미 30분이다).
                        숫자를 비율로 적은 것은 데이터가 늘어도 안내가 틀리지 않게 하려는 것이다. */}
                    <div className="text-xs text-surface-400 dark:text-surface-500 text-right max-w-[16rem]">
                        매일 새벽 2시에 자동으로 갱신됩니다.
                        <br />
                        직접 갱신하면 하루 무료 조회 한도의 3분의 1가량을 쓰니, 지금 꼭 필요할 때만 눌러 주세요.
                    </div>
                </div>
            </div>
