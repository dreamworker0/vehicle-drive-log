 ...
 * 그 시점 이후 **공유가 살아났는지 죽었는지 아무도 모른다.** 이 패널은 서비스 계정
 * 신원으로 지금 다시 물어, 카운터만 되돌리면 되는 기관과 재연동이 필요한 기관을 가른다.
 *
...
interface ProbeSummary {
    probedCalendars: number;
    probedVehicles: number;
    okCalendars: number;
    resettableCalendars: number;
    resettableVehicles: number;
    needsOrgActionCalendars: number;
    needsOrgActionVehicles: number;
    falsePositiveCalendars: number;
...
export default function DashboardCalendarProbe() {
    ...
    const runProbe = useCallback(async () => {
        ...
        try {
            ...
            const fn = httpsCallable<{ includeHealthy: boolean }, ProbeResponse>(functions, 'probeCalendarAccess');
            const res = await fn({ includeHealthy });
            setData(res.data);
        } catch (err: unknown) {
            showToast('진단 실패: ' + ((err as Error).message || '알 수 없는 오류'), 'error');
        } finally {
            setLoading(false);
        }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                        <div className="p-3 rounded-xl border bg-accent-50 dark:bg-accent-900/20 border-accent-200 dark:border-accent-800/40">
                            ...
                            <p className="text-lg font-bold text-accent-600 dark:text-accent-400">
                                ...
                                <span className="ml-1 text-xs font-normal">/ 캘린더 {summary.resettableCalendars}개</span>
                            </p>
                        </div>
                        <div className="p-3 rounded-xl border bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800/40">
                            <p className="text-xs text-surface-500 dark:text-surface-400 mb-1">기관 재연동 필요</p>
                            <p className="text-lg font-bold text-red-600 dark:text-red-400">
                                {summary.needsOrgActionVehicles}대
                                <span className="ml-1 text-xs font-normal">/ 캘린더 {summary.needsOrgActionCalendars}개</span>
                            </p>
                        </div>
                        <div className="p-3 rounded-xl border bg-surface-50 dark:bg-surface-800 border-surface-200 dark:border-surface-700">
                            ...
                            <p className="text-lg font-bold text-surface-600 dark:text-surface-300">
                                ...
                                <span className="ml-1 text-xs font-normal">/ 캘린더 {summary.falsePositiveCalendars}개</span>
                            </p>
                        </div>
                    </div>

                    <p className="text-xs text-surface-500 dark:text-surface-400 mb-2">
                        캘린더 {summary.probedCalendars}개 · 차량 {summary.probedVehicles}대 진단
                        {summary.truncated && (
                            <span className="ml-1 text-amber-600 dark:text-amber-400">
                                (상한 초과 — 전체 {summary.totalCalendars}개 중 일부만 진단했습니다)
                            </span>
                        )}
                    </p>
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="text-left text-surface-500 dark:text-surface-400 border-b border-surface-100 dark:border-surface-700">
                                    <th className="py-2 pr-3 font-medium whitespace-nowrap">기관</th>
                                    <th className="py-2 pr-3 font-medium whitespace-nowrap">판정</th>
                                    <th className="py-2 pr-3 font-medium whitespace-nowrap">차량</th>
                                    <th className="py-2 pr-3 font-medium whitespace-nowrap">실패</th>
                                    <th className="py-2 pr-3 font-medium">필요한 조치</th>
                                </tr>
                            </thead>
                            <tbody>
                                {data.rows.map((row) => {
                                    const cfg = VERDICT_CONFIG[row.verdict];
                                    const inert = !row.calendarEnabled || row.organizationStatus !== 'approved';
                                    return (
                                        <tr
                                            key={row.calendarId}
                                            ...
                                            <td className="py-2 pr-3 text-surface-600 dark:text-surface-300">
                                                {inert ? (
                                                    <span className="text-surface-400 dark:text-surface-500">
                                                        ...
                                                        {row.organizationStatus !== 'approved' && ` (기관 상태: ${row.organizationStatus})`}
                                                    </span>
                                                ) : (
                                                    cfg.action
                                                )}
                                                {row.detail && row.verdict === 'error' && (
