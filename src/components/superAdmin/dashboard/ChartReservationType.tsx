...
function ChartReservationType({ reservationTypeStats, reservationTypeRatio }: Props) {
            ...
            <div className="glass-card p-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-3 bg-surface-50 dark:bg-surface-800/50 p-3 rounded-xl border border-surface-200 dark:border-surface-700">
                        <div className="text-center">
                            ...
                            <div className="text-sm font-bold text-surface-700 dark:text-surface-300">{reservationTypeRatio.total.toLocaleString()}건</div>
                        </div>
                        <div className="w-px h-8 bg-surface-200 dark:bg-surface-700"></div>
                        <div className="text-center">
                            <div className="text-[11px] text-blue-500 mb-0.5">하루</div>
                            <div className="text-sm font-bold text-blue-600 dark:text-blue-400">{reservationTypeRatio.singleRate}%</div>
                        </div>
                        <div className="w-px h-8 bg-surface-200 dark:bg-surface-700"></div>
                        <div className="text-center">
                            <div className="text-[11px] text-amber-500 mb-0.5">다일</div>
                            <div className="text-sm font-bold text-amber-600 dark:text-amber-400">{reservationTypeRatio.multiDayRate}%</div>
                        </div>
                        <div className="w-px h-8 bg-surface-200 dark:bg-surface-700"></div>
                        <div className="text-center">
                            <div className="text-[11px] text-purple-500 mb-0.5">반복</div>
                            <div className="text-sm font-bold text-purple-600 dark:text-purple-400">{reservationTypeRatio.recurringRate}%</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-6">
                    <ResponsiveContainer width="100%" height={256} minWidth={1}>
                        <AreaChart data={reservationTypeStats} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                            <defs>
                            ...
                            <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false}
                                axisLine={{ stroke: '#4b5563' }} interval={Math.ceil(reservationTypeStats.length / 8)} />
    ...
    }, [reservationTypeStats, reservationTypeRatio]);
