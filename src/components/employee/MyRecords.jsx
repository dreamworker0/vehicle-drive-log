            ...
            {showSearch && (
                <div className="relative mb-3 animate-fade-in">
                    ...
                    <input
                        ...
                        placeholder="차량명, 행선지, 목적, 비고 검색…"
                        className="w-full pl-9 pr-9 py-2.5 rounded-xl bg-surface-100 dark:bg-surface-700 text-sm text-surface-900 dark:text-surface-100 placeholder-surface-400 border border-surface-200 dark:border-surface-600 focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 transition-colors"
                    />
                    {searchQuery && (
                        <button
                            ...
                            <div className="space-y-2">
                                {monthLogs.map(log => (
                                    ...
                                    >
                                        ...
                                        >
                                            <div className={`w-10 h-10 rounded-xl ${log.vehicleId ? getVehicleColor(log.vehicleId) : 'bg-primary-50'} flex items-center justify-center text-lg flex-shrink-0`}>
                                                {VEHICLE_TYPE_ICONS[log.vehicleType] || '🚗'}
                                            </div>
                                        ...
                                        {expandedId === log.id && (
                                            <div className="px-4 pb-4 border-t border-surface-100 dark:border-surface-700 pt-3 animate-fade-in">
                                                <div className="grid grid-cols-2 gap-3 text-sm">
                                                    <div>
                                                        <span className="text-xs text-surface-400">출발 km</span>
                                                        <p className="font-mono text-surface-700 dark:text-surface-300">{log.startKm?.toLocaleString()} km</p>
                                                    </div>
                                                    <div>
                                                        <span className="text-xs text-surface-400">도착 km</span>
                                                        <p className="font-mono text-surface-700 dark:text-surface-300">{log.endKm?.toLocaleString()} km</p>
                                                    </div>
                                                    {(log.startTime || log.endTime) && (
                                                        <div className="col-span-2">
                                                            <span className="text-xs text-surface-400">운행 시각</span>
                                                            <p className="text-surface-700 dark:text-surface-300 font-mono">
                                                                {log.startTime || '--:--'} → {log.endTime || '--:--'}
                                                            </p>
                                                        </div>
                                                    )}
                                                    {log.purpose && (
                                                        <div>
                                                            <span className="text-xs text-surface-400">운행 목적</span>
                                                            <p className="text-surface-700 dark:text-surface-300">{log.purpose}</p>
                                                        </div>
                                                    )}
                                                    {log.destination && (
                                                        <div>
                                                            <span className="text-xs text-surface-400">행선지</span>
                                                            <p className="text-surface-700 dark:text-surface-300">{log.destination}</p>
                                                        </div>
                                                    )}
                                                    {log.fuelAmount && (
                                                        <div>
                                                            <span className="text-xs text-surface-400">주유비</span>
                                                            <p className="text-surface-700 dark:text-surface-300">{Number(log.fuelAmount).toLocaleString()}원</p>
                                                        </div>
                                                    )}
                                                    {log.batteryStart != null && (
                                                        <div className="col-span-2">
                                                            <span className="text-xs text-surface-400">배터리</span>
                                                            <p className="text-surface-700 dark:text-surface-300">🔋 {log.batteryStart}% → {log.batteryEnd}%</p>
                                                        </div>
                                                    )}
                                                    {log.passengerCount > 0 && (
                                                        <div className="col-span-2">
                                                            <span className="text-xs text-surface-400">탑승인원</span>
                                                            <p className="text-surface-700 dark:text-surface-300">
                                                                👥 {log.passengerCount}명
                                                                {log.passengerNames?.length > 0 && (
                                                                    <span className="text-xs text-surface-400 ml-1">({log.passengerNames.join(', ')})</span>
                                                                )}
                                                            </p>
                                                        </div>
                                                    )}
                                                    {log.notes && (
                                                        <div className="col-span-2">
                                                            <span className="text-xs text-surface-400">비고</span>
                                                            <p className="text-surface-700 dark:text-surface-300">{log.notes}</p>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="flex gap-2 mt-3">
                                                    <button
                                                        onClick={() => navigate('/employee/drive-log', { state: { continueFrom: log } })}
                                                        className="btn-soft-primary flex-1 text-sm"
                                                    >
                                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                                                    ...
                                                    <button
                                                        onClick={() => navigate('/employee/drive-log', { state: { editLog: log } })}
                                                        className="btn-soft-blue flex-1 text-sm"
                                                    >
                                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
