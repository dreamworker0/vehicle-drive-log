        ...
        <div className="max-w-lg mx-auto animate-fade-in">
            <div className="flex items-center justify-between mb-4">
                ...
                <button
                    ...
                    className="btn-primary text-sm !py-2 !px-3 min-h-[48px]"
                >
                    {showForm ? '취소' : '+ 추가'}
                </button>
            </div>
            ...
            {showForm && (
                <form onSubmit={handleAdd} className="glass-card p-4 mb-4 space-y-3 animate-fade-in">
                    <div>
                        <label className="label">이름 (별칭) <span className="text-red-500 dark:text-red-400">*</span></label>
                        <input
                            type="text"
                            value={newName}
                    ...
                    <button
                        ...
                        className="btn-primary w-full text-sm min-h-[48px]"
                    >
                        {adding ? '추가 중...' : '즐겨찾기 추가'}
                    </button>
                </form>
            ...
            {favorites.length === 0 ? (
                <div className="glass-card p-12 text-center">
                    ...
                    <p className="text-surface-400 dark:text-surface-500 font-medium">등록된 즐겨찾기가 없습니다</p>
                    <p className="text-xs text-surface-300 dark:text-surface-600 mt-1">자주 가는 목적지를 추가해보세요</p>
                </div>
            ) : (
                    ...
                    {favorites.map(fav => (
                        <div key={fav.id} className="glass-card p-4 flex items-center gap-3">
                            ...
                            <button
                                ...
                                className="btn-icon text-surface-300 dark:text-surface-600 hover:text-red-500 dark:hover:text-red-400 min-w-[48px] min-h-[48px] flex items-center justify-center flex-shrink-0"
                                title="삭제"
                                aria-label="즐겨찾기 삭제"
                            >
