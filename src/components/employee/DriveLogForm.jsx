...
export default function DriveLogForm() {
    ...
    const subtitle = isEditMode
        ? `${editLog.vehicleName || '차량'} · ${editLog.destination || ''} 기록을 수정합니다`
        : continueFrom
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1">
                                ...
                                {form.destination.trim() && !favorites.some(f => f.address === form.destination.trim() || f.name === form.destination.trim()) && (
                                    <button
                                        type="button"
                            ...
                            {/* 즐겨찾기 저장 폼 */}
                            {showFavSave && (
                                <div className="mt-2 p-3 rounded-lg bg-amber-50 border border-amber-200 animate-fade-in">
                ...
                {members.length > 0 && (
                    <div className="glass-card p-4">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {members.map(m => {
                                const isSelected = selectedPassengers.some(p => p.id === m.id);
                                return (
                                    <button
                                        key={m.id}
                                        type="button"
