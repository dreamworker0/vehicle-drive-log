...
export default function MorePage() {
    ...
    const [insuranceVehicles, setInsuranceVehicles] = useState([]);
    const notifApiAvailable = typeof window !== 'undefined' && 'Notification' in window;

    // 보험 정보 패널 열릴 때 차량 데이터 로드
    useEffect(() => {
        if (!showInsurance || !userData?.organizationId) return;
        getVehicles(userData.organizationId).then(list => {
    ...
    return (
        <div className="max-w-lg mx-auto animate-fade-in space-y-3">
            ...
            <div className="glass-card p-4 flex items-center gap-3">
                ...
                <button
                    onClick={logout}
                    className="flex-shrink-0 flex items-center gap-1.5 text-xs text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 px-3 py-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"
                >
            ...
            <div className="glass-card divide-y divide-surface-100 dark:divide-surface-700">
                <button
                    ...
                    className="w-full flex items-center gap-3 p-4 hover:bg-surface-50 dark:hover:bg-surface-700 transition-colors text-left"
                >
                ...
                <button
                    onClick={() => navigate('/employee/vehicle-history')}
                    className="w-full flex items-center gap-3 p-4 hover:bg-surface-50 dark:hover:bg-surface-700 transition-colors text-left"
                >
                    <svg className="w-5 h-5 text-surface-400" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                ...
                >
                    ...
                    <svg className={`w-4 h-4 ml-auto text-surface-400 transition-transform ${showInsurance ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                                    ...
                                    <div key={v.id} className="p-3 rounded-xl bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700">
                                        <div className="flex items-center gap-2.5">
                                            ...
                                            <div className="ml-auto flex items-center gap-2 text-xs flex-shrink-0">
                                                ...
                                                {v.insurance.phone && (
                                                    <a href={`tel:${v.insurance.phone}`} className="text-primary-500 font-medium hover:underline">📞 {v.insurance.phone}</a>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                ...
                <button
                    ...
                    className="w-full flex items-center gap-3 p-4 hover:bg-primary-50 dark:hover:bg-primary-900/30 transition-colors text-left"
                >
                ...
                {notifApiAvailable && (
                    <div
                        className="flex items-center justify-between p-4 cursor-pointer hover:bg-surface-50 dark:hover:bg-surface-700 transition-colors"
                        onClick={() => {
                ...
                <div className="flex items-center justify-between p-4">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <span className="text-sm text-surface-700 dark:text-surface-300">길안내 앱</span>
                            <p className="text-[11px] text-surface-400 dark:text-surface-500">네이버맵만 경유지 가능</p>
                        </div>
                    </div>
                    <div className="flex rounded-lg border border-surface-200 dark:border-surface-600 overflow-hidden">
                        {[{ key: 'naver', label: '네이버' }, { key: 'kakao', label: '카카오' }, { key: 'tmap', label: '티맵' }].map(opt => (
                            <button
                                key={opt.key}
                                onClick={() => {
                                    try { localStorage.setItem('preferred-nav-app', opt.key); } catch { /* noop */ }
                                    setNavApp(opt.key);
                                }}
                                className={`px-3 py-1.5 text-xs font-medium transition-colors ${navApp === opt.key
                    ...
                    <div className="flex rounded-lg border border-surface-200 dark:border-surface-600 overflow-hidden">
                        {[{ key: 'small', label: '작게' }, { key: 'normal', label: '보통' }, { key: 'large', label: '크게' }].map(opt => (
                            <button
                                ...
                                className={`px-3 py-1.5 text-xs font-medium transition-colors ${fontSize === opt.key
                                    ? 'bg-primary-600 text-white'
                                    : 'bg-surface-50 dark:bg-surface-700 text-surface-600 dark:text-surface-300 hover:bg-surface-100 dark:hover:bg-surface-600'
                                    }`}
                            >
                ...
                <button
                    onClick={async () => {
                        if (!confirm('정말 이 기관에서 나가시겠습니까?\n나가면 다시 초대 코드를 입력해야 합니다.')) return;
            ...
            <div className="mt-6 flex items-center justify-center gap-2 text-xs text-surface-400">
                ...
                <span>제작자: 김종원</span>
                <span>·</span>
                <a href="mailto:ehsheh@gmail.com" className="text-primary-500 hover:text-primary-700 transition-colors">
                    ehsheh@gmail.com
                </a>
            </div>
