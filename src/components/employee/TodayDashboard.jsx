...
export default function TodayDashboard() {
    ...
    return (
        <div className="max-w-lg mx-auto animate-fade-in">
            ...
            {/* 임박 예약 알림 배너 */}
            {upcomingAlerts.filter(r => !dismissedAlerts.has(r.id)).length > 0 && (
                <div className="mb-4 space-y-2">
                    {upcomingAlerts.filter(r => !dismissedAlerts.has(r.id)).map(r => (
            ...
            {incompleteAlerts.length > 0 && (
                <div className="mb-4 space-y-2">
                    {incompleteAlerts.map((alert, idx) => (
                        <div key={alert.id || idx} className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700/50 rounded-xl p-3 flex items-center gap-3 animate-fade-in">
                            ...
                            <button
                                ...
                                className="btn-sm text-xs whitespace-nowrap bg-red-100 text-red-700 hover:bg-red-200"
                            >
                                작성하기
                            </button>
                        </div>
