                        ...
                        return (
                            <div key={card.id} className="glass-card p-5 transition-all group hover:shadow-glass-lg">
                                <div className="flex items-start justify-between mb-3">
                                    ...
                                    <div className="flex gap-1 opacity-100 pointer-fine:opacity-0 pointer-fine:group-hover:opacity-100 pointer-fine:group-focus-within:opacity-100 transition-opacity">
                                        <button onClick={() => handleEdit(card)} aria-label={`${card.vehicleName || vehicle?.displayName || '차량 미연결'} 하이패스 수정`} className="btn-icon btn-sm text-surface-400 dark:text-surface-500 hover:text-primary-600 dark:hover:text-primary-400 min-h-[48px] min-w-[48px]">
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                                                <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z" />
                                            </svg>
                                        </button>
                                        <button onClick={() => openDeleteModal(card)} aria-label={`${card.vehicleName || vehicle?.displayName || '차량 미연결'} 하이패스 삭제`} className="btn-icon btn-sm text-surface-400 dark:text-surface-500 hover:text-red-500 dark:hover:text-red-400 min-h-[48px] min-w-[48px]">
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
