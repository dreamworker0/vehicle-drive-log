...
import FuelLogEditForm from './fuelLog/FuelLogEditForm';
                            ...
                            <div key={rec.id} className={`glass-card p-4 hover:shadow-glass-lg transition-all ${editingRecord?.id === rec.id ? 'ring-2 ring-primary-400' : ''}`}>
                                ...
                                <div className="hidden sm:grid gap-2 items-center" style={{ gridTemplateColumns: GRID_COLUMNS }}>
                                    <div>
                                        ...
                                        {/* 배지는 날짜 아래에 둔다 — 주유원 칸에 붙이면 폭이 좁아 이름이 잘린다.
                                            '누가 고쳤나'는 사람이 아니라 기록에 대한 표시이기도 하다. */}
                                        <AdminEditedBadge lastEditedByUid={rec.lastEditedByUid} ownerUid={rec.driverUid} className="mt-0.5" />
                                    </div>
                                    <div>
