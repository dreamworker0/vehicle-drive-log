...
const STEPS = [
    ...
    },
    {
        icon: '🔑',
        title: '초대 코드를 공유하세요',
        desc: '직원들에게 아래 초대 코드를 알려주세요.\n코드를 입력하면 기관에 자동으로 연결됩니다.',
        action: null,
        path: null,
    },
    {
        icon: '✅',
        title: '준비 완료!',
        desc: '직원들이 코드를 입력하고 앱에 접속하면\n바로 차량 예약과 운행일지 작성이 가능합니다.',
        action: null,
        path: null,
    },
];

export default function AdminOnboardingWizard({ inviteCode, onDismiss }) {
        ...
        <div className="glass-card border-2 border-primary-200 p-6 mb-6 animate-fade-in">
            ...
            <div className="flex gap-1.5 mb-6">
                {STEPS.map((_, i) => (
                    <div
                        key={i}
                        className={`h-1 flex-1 rounded-full transition-colors duration-300 ${i <= step ? 'bg-primary-500' : 'bg-surface-200'
                            }`}
                    />
                ))}
            </div>
...
/** 위자드를 표시할지 판단하는 헬퍼 */
AdminOnboardingWizard.shouldShow = (vehicleCount, employeeCount) => {
    try {
