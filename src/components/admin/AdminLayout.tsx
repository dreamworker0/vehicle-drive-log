...
function NavItem({ to, icon, label, badge, onClick }: NavItemProps) {
    return (
        ...
        >
            ...
            <span>{label}</span>
            {badge != null && (
                <span className={`ml-auto inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-xs font-semibold rounded-full ${badge > 0 ? 'bg-primary-100 text-primary-700 dark:bg-primary-900/40 dark:text-primary-300' : 'bg-surface-100 text-surface-400 dark:bg-surface-800 dark:text-surface-500'}`}>
                    {badge}
                </span>
            )}
        </NavLink>
...
export default function AdminLayout() {
    const { user, isSuperAdmin, orgFeatures } = useAuth();
    ...
    const menuItems = [
        ...
        // 기관 설정에서 꺼진 기능의 메뉴는 숨긴다
        if (item.to === '/admin/hipass' && !orgFeatures.hipass) return false;
        if (item.to === '/admin/maintenance' && !orgFeatures.maintenance) return false;
        return true;
    });
            ...
            <main className="flex-1 flex flex-col min-w-0" role="main">
                <header className="sticky top-0 z-30 bg-white/80 dark:bg-surface-900/80 backdrop-blur-md border-b border-surface-100 dark:border-surface-700 px-4 lg:px-6 h-14 flex items-center justify-between safe-top">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            onClick={() => { markReleaseNotesSeen(); setShowReleaseNotes(true); }}
                            className="btn-icon relative min-h-[48px] min-w-[48px] text-surface-500 dark:text-surface-400 hover:text-primary-600 dark:hover:text-primary-400"
                            title="업데이트 소식"
                        >
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
