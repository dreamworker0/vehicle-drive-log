...
| 층 | 방법 |
|---|---|
| 서버 | `lastUpdatedAt` **1건 read**로 쿨다운 판정. 이 1건이 풀스캔을 막는 값이다. 쿨다운 길이의 단일 원본은 `functions/src/handlers/callable/refreshDashboardStats.ts`의 `REFRESH_COOLDOWN_SEC`(현재 30분)이므로 **문서 값을 인용하지 말고 코드를 확인한다** |
