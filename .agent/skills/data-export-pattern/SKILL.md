...
1.  **용지 사양**: A4 가로 기준 1페이지 행 수(`ROWS_PER_PAGE`)를 설정합니다(보통 19 ~ 25행).
    *   ⚠️ **셀이 줄바꿈되는 표에는 고정 행 수를 쓰지 마세요.** `td`의 `height`는 *최소* 높이라서 목적지 주소처럼 긴 값이 오면 행이 2~3줄로 자랍니다. 고정 분할한 묶음이 용지 한 장을 넘기면 뒤쪽 행이 **제목도 머리글도 없는 다음 장**으로 밀립니다(21건이 용지 3장으로 나갔던 실제 사례).
    *   운행일지(`pdfExport.ts`)는 [`pageFit.ts`](../../../src/lib/pdf/pageFit.ts)로 **실제 행 높이를 재서** 나눕니다 — 전체 행을 담은 측정용 문서를 인쇄 폭(277mm)과 같은 숨은 iframe에 그려 행별 높이를 재고, 남은 높이만큼만 담습니다. 측정이 불가능한 환경에서는 고정 행 수로 되돌아갑니다.
    *   새 출력물의 표에 자유 입력 텍스트 열(목적지·비고 등)이 있으면 같은 방식을 쓰세요. 열이 전부 짧은 고정 폭(날짜·금액 등)이면 고정 행 수로 충분합니다.
2.  **결재란 공통 구현**: 모든 출력물 우측 상단에 결재란 테이블을 자동 삽입합니다.
3.  **데이터 패딩**: 데이터가 부족한 페이지는 레이아웃 깨짐을 방지하기 위해 빈 행(`&nbsp;`)으로 채웁니다. 실측 분할에서는 빈 행 수도 남은 높이에서 계산합니다(빈 행을 채워 용지를 넘기면 같은 문제가 재발합니다).
