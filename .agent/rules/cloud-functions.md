...
- **Firebase**: `firebase-admin` 14, `firebase-functions` 7 (2nd gen)
...
### 3.5 되돌릴 수 없는 대량 발송 — 순서와 안전장치

전체 기관 일괄 공지처럼 **되돌릴 수 없는 대량 행위**는 기능보다 "실수하기 어렵게 만드는 것"이 설계의 핵심이다(구현이력 Phase 134).
...
뒤집으면 **푸시를 받고 들어온 사용자의 알림함이 비어 있을 수 있다.** 이 순서는 테스트로 고정한다(커밋 2회 → 푸시 2회).

**그 밖의 필수 장치:**

- **2단계 강제** — `dryRun`이 대상 수만 반환하고 화면이 그 인원을 표시한 뒤에만 발송 버튼이 열린다. "몇 명에게 가는지 모르고" 누르는 상황을 없앤다
- **확인한 문안 = 보내는 문안** — 제목·본문을 고치면 확인 결과를 버린다. **A를 확인하고 B를 보내는 것이 이 화면의 최대 위험이다**
- **중복 차단** — 문서 ID를 `broadcast_{noticeId}_{uid}`로 고정해 재클릭·재시도가 같은 사람에게 두 번 띄우지 않는다. 화면이 발송마다 새 `noticeId`를 만들므로 **의도적 재발송은 막지 않는다**
- **부분 실패를 되돌리지 않는다** — 푸시 청크 실패는 삼키고 성공/실패 수만 반환한다. 전체를 되돌리면 일부만 받은 상태에서 재발송해야 한다
- **500건 상한** — Firestore 배치·FCM `sendEach` 모두. 실측으로 고정한다(1,201명 → 500·500·201)
- **읽기 N+1 제거** — 기관 단위 헬퍼를 200여 기관에 반복하면 users 쿼리가 기관 수만큼 돈다. users를 한 번만 훑고 그때 얻은 `fcmToken`으로 `sendEach`를 쓴다
- 비활성·기관 미소속 계정은 제외하되, `status` 없는 구 문서는 활성으로 간주한다
...
배포는 로컬에서 직접 하지 않는다 — **master 푸시 시 CI(Deploy 워크플로)가 수행한다.** 로컬 `firebase deploy`를 병행하면 동일 함수 동시 업데이트 충돌이 발생한다. 긴급 시에만 CI 미실행을 확인하고 [/deploy-emergency 워크플로우](../workflows/deploy-emergency.md)를 따른다 (Node 22 필수).
