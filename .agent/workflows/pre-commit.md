...
| `pre-push` | `npm run test` (빌드·보안 감사는 CI가 한다) |
...
- `git push` 실행 시 → `.husky/pre-push`가 단위 테스트 실행 → 실패하면 푸시 차단 (빌드·보안 감사는 CI에서)
- 상세 규칙: [.agent/rules/pre-commit.md](../rules/pre-commit.md)

## 주의사항

- pre-commit은 **빠른 안전망**(타입 검사 + 스테이징 파일 lint/테스트), 전체 검증은 `npm run verify:full` 또는 `/deploy` 워크플로우.
