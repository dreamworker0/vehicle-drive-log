...
수정은 F-01~F-05만 작은 단위 PR로 분리하고, F-06은 리포트에만 남긴다.
...
1. `fix: 운행일지 운전자와 차량 참조 검증 강화` — F-01, F-02와 Rules 회귀 테스트
2. `fix: 비활성 기관 기능의 직접 쓰기 차단` — F-03과 Rules 회귀 테스트
3. `fix: 캘린더 비활성 기관 동기화 중단` — F-04와 Functions 단위 테스트
4. `fix: 기능 토글 동시 저장 유실 방지` — F-05와 hook 단위 테스트
