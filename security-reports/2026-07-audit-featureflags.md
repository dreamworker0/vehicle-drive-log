...
- **F-04·F-05는 수정 완료** — F-04는 PR #42(리뷰 대기), F-05는 PR #41(머지 완료).
- **F-01·F-02·F-03은 Rules 강제 대신 "잔여 위험 수용"으로 문서화** — 셋 다 정상 UI가 이미 제약하고, 악용에는
  인증된 내부 사용자의 raw SDK 요청 조작이 필요하며 영향이 자기 기관 내부에 그친다. 상세 근거·재검토
  트리거는 각 항목의 "조치 결정" 참고.
- **F-06은 리포트에만** 남긴다.
...
- 제안 수정: 미설정은 ON으로 보는 공통 Rules 헬퍼를 추가한다. 정비는 `maintenanceEnabled`와
  ...
  레거시 문서가 잠기지 않게 한다.
- **조치 결정(2026-07-17): 잔여 위험 수용 — Rules 강제하지 않음.** 근거:
  1. F-01·F-02와 동일한 성격이다 — 정상 UI는 이미 비활성 기능의 탭·입력을 숨겨(`FuelLogTab`, `DriveLogForm`
     게이트) 앱 사용자는 접근할 수 없고, 악용에는 인증된 내부 사용자의 raw SDK 요청 조작이 필요하다.
     영향은 자기 기관에 "꺼둔 기능"의 데이터가 쌓이는 정도로 교차 테넌트 유출이 아니다.
  2. Rules 강제 자체는 저볼륨 컬렉션(정비·하이패스)이라 org get() 1회로 비용이 감당되지만, 방어할 실제
     위협이 사실상 없고(소규모 기관 1~2인 사용), 에뮬레이터 RED/GREEN 검증 인프라 도입 비용이 이득을
     넘어선다고 판단했다.
  - 재검토 트리거: 비활성 기능의 직접 쓰기가 실제로 관측되거나 기능 OFF의 의미가 규정/감사 요건으로
    격상되면 → 위 제안 수정을 저볼륨 컬렉션부터 단계적으로 적용한다.
...
2. ~~`fix: 비활성 기관 기능의 직접 쓰기 차단` — F-03~~ → **잔여 위험 수용으로 결정, PR 없음**(위 조치 결정 참고)
