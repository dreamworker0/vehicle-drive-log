 ...
 * **페이지 진입마다** 익명 로그인을 걸었고, git 이력이 시작되는 첫 커밋부터 있었다
 * (`1e1cba8`, 2026-03-05 = 최초 커밋이므로 실제 도입은 그 이전이다). **2026-04-08에
 * 제거**했다(`0b33657`) — 지금은 서버가 Admin SDK로 업로드하고 `applicantUid:
 * "anonymous-app"`이 그 자리를 대신한다.
 *
 * 2026-08-24 실측(프로덕션): 전체 1837개 중 익명 9개, **전부 참조 0건**. 생성 시각이
 * 2026-02-24(5개, 18분 내) · 2026-07-14(3개, 9분 내) · 2026-08-10(1개)로 뭉쳐 있고 모두
 * 생성 = 최근 로그인이었다 — 한 사람이 신청 페이지를 여러 번 여는 모양이다. 2월은 코드가
 * 살아 있던 시기라 그대로 설명되고, 제거 이후인 7·8월은 **서비스워커 캐시에 옛 번들이
 * 남은 기기**가 유력하다(공개 API 키로 발급을 시도한 흔적일 수도 있다).
 * 어느 쪽인지는 참조 조회로 가른다 — 아래 안전 장치 2번.
 *
