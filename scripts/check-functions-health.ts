...
function isDeployAuditLog(body: string): boolean {
    return body.includes('google.cloud.audit.AuditLog');
}
...
export function summarizeLogs(raw: string): HealthReport {
    ...
    for (const line of lines) {
        ...
        if (kind === 'deploy') {
            ...
            if (body.includes('Quota exceeded')) report.deployQuotaEvents++;
            continue;
        }
...
function render(report: HealthReport, totalLines: number): void {
    ...
    if (totalLines > 0 && report.parsed === 0) {
    ...
    if (report.appErrors === 0 && report.requestErrors > 0) {
        console.log('   ↳ 전부 4xx/5xx 응답 로그다. 함수가 남긴 에러는 없으니, 그 상태 코드가 정상 거절인지부터 확인하라.');
    }
}
