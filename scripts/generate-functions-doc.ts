...
const FUNCTIONS: FunctionEntry[] = [
  ...
  },
  {
    ...
    description: '통합 야간 배치: 월간 집계 캐싱 → 대시보드 통계 재집계 → Firestore 백업(GCS) → soft-delete 기관 30일 후 영구 삭제 → 인증서 이미지 정리 → 3년 이상 운행기록 아카이빙 → 차량 보험 만료 알림',
    auth: '시스템 자동 실행',
    note: 'schedule: "0 2 * * *" (KST 02:00). 개별 스케줄러(backupFirestore·autoPurgeOrgs·archiveDriveLogs·cleanupCertificateImages)를 통합해 Cloud Scheduler 잡 수를 줄인 것',
  },
  {
    name: 'monthlyBatch',
    type: 'onSchedule',
