...
const FUNCTIONS: FunctionEntry[] = [
  ...
  {
    name: 'onHipassChargeCreated',
    type: 'onDocumentCreated',
    file: 'handlers/triggers/syncHipassBalance.ts',
    description: '충전 기록이 생기면 그만큼 하이패스 카드 잔액을 올린다(트랜잭션). 잔액의 주인은 서버다 — 클라이언트는 쓰지 않는다.',
    auth: '시스템 자동 실행',
    note: 'hipassCharges/{chargeId}',
  },
  {
    name: 'onHipassChargeUpdated',
    type: 'onDocumentUpdated',
    file: 'handlers/triggers/syncHipassBalance.ts',
    description: '충전 기록이 고쳐지면 차액만큼 잔액을 맞춘다. 카드를 옮긴 정정은 옛 카드에서 빼고 새 카드에 더한다.',
    auth: '시스템 자동 실행',
    note: 'hipassCharges/{chargeId}',
  },
  {
    name: 'onHipassChargeDeleted',
    type: 'onDocumentDeleted',
    file: 'handlers/triggers/syncHipassBalance.ts',
    description: '충전 기록이 지워지면 그만큼 잔액을 되돌린다(삭제 확인창이 약속하는 그 동작).',
    auth: '시스템 자동 실행',
    note: 'hipassCharges/{chargeId}',
  },
  {
