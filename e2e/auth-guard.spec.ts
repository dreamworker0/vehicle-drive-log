...
test.describe('AuthGuard 작동 (인증 및 권한 라우터 가드)', () => {
    test('로그인하지 않은 상태에서 보호된 라우트(/admin) 접근 시 로그인 페이지로 리다이렉트된다', async ({ page }) => {
        ...
        // 랜딩 페이지의 "로그인" 버튼 또는 "차량 운행일지" 제목이 보이면 리다이렉트 성공
        const loginBtn = page.getByRole('button', { name: '로그인' });
        const heroTitle = page.getByRole('heading', { name: /차량 운행일지/ });
        await expect(loginBtn.or(heroTitle)).toBeVisible({ timeout: 10000 });
    });
